...
bool BlurFullyVisible = false;
float BlurTransparency = 0.0f;
//bool EmptyCrosshairSet = false;
...
int CHudCrosshairStatic::Init( void )
{
	...
	SniperCrosshairBlur = 0;
	CrossbowCrosshairImage = 0;
	CrossbowCrosshairBlur = 0;
	AlienCrosshairImage = 0;
	AlienCrosshairBlur = 0;
	Reset();
	return 1;
}
...
int CHudCrosshairStatic::VidInit( void )
{
	...
	SniperCrosshairBlur = LOAD_TEXTURE( "sprites/scope_sniper_blur.dds", NULL, 0, 0 ); // HACKHACK :|
	CrossbowCrosshairImage = LOAD_TEXTURE( "sprites/scope_crossbow.dds", NULL, 0, 0 );
	CrossbowCrosshairBlur = LOAD_TEXTURE( "sprites/scope_crossbow_blur.dds", NULL, 0, 0 );
	AlienCrosshairImage = LOAD_TEXTURE( "sprites/scope_alien.dds", NULL, 0, 0 );
	AlienCrosshairBlur = LOAD_TEXTURE( "sprites/scope_alien_blur.dds", NULL, 0, 0 );
	G36CCrosshairImage = LOAD_TEXTURE( "sprites/scope_g36c.dds", NULL, 0, 0 );
	G36CCrosshairBlur = LOAD_TEXTURE( "sprites/scope_g36c_blur.dds", NULL, 0, 0 );
	...
	if( ShouldEnable > 0 )
	{
		...
		if( ConfirmedHit > 0 )
		{
			...
			if( CVAR_TO_BOOL(cl_hitsound) )
			{
				switch( ConfirmedHit )
				{
				case 1: PlaySound( "misc/hitmarker.wav", 1.0 ); break;
				case 2: PlaySound( "misc/hitmarker_kill.wav", 1.0 ); break;
				}
			}
		}
	}
...
int CHudCrosshairStatic::Draw( float flTime )
{
	...
	if( gHUD.IsZoomed == true ) // draw sniper scope
	{
		...
		if( gHUD.IsZooming )
		//	BlurTransparency += 5 * g_fFrametime;
			BlurTransparency = 2.0f;
		else
		{
			...
			{
				if( YawSpeed > 20 )
					BlurTransparency += 20 * g_fFrametime;
				else
					BlurTransparency -= 5 * g_fFrametime;
			}
			else
				BlurTransparency = Velocity * 0.01;
		}
		BlurTransparency = bound( 0.0f, BlurTransparency, 2.0f );
		
		// :D
		// need to make sure that the blurred image is fully opaque, then we can subtract from the crosshair
		if( BlurTransparency > 0.99f )
			BlurFullyVisible = true;
		else
			BlurFullyVisible = false;
		
		gEngfuncs.pTriAPI->RenderMode( kRenderTransAlpha );
		gEngfuncs.pTriAPI->Brightness( 1.0f );
		if( BlurFullyVisible )
			gEngfuncs.pTriAPI->Color4f( 1, 1, 1, 2.0f - BlurTransparency );
		else
		...
		// an experiment - draw an image with blur, if player moves
		DrawBlurredScope( WeaponID, BlurTransparency );
...
void CHudCrosshairStatic::DrawBlurredScope( int WeaponID, float BlurTransparency )
{
	if( BlurTransparency < 0.1f )
		return;

	BlurTransparency = bound( 0.0f, BlurTransparency, 1.0f );

	if( WeaponID == WEAPON_SNIPER )
		GL_Bind( 0, SniperCrosshairBlur );
	else if( WeaponID == WEAPON_CROSSBOW )
		GL_Bind( 0, CrossbowCrosshairBlur );
	else if( WeaponID == WEAPON_GAUSS )
		GL_Bind( 0, AlienCrosshairBlur );
	else if( WeaponID == WEAPON_G36C )
		GL_Bind( 0, G36CCrosshairBlur );
	else
		return;

	gEngfuncs.pTriAPI->RenderMode( kRenderTransAlpha );
	gEngfuncs.pTriAPI->Color4f( 1, 1, 1, BlurTransparency );
	gEngfuncs.pTriAPI->Begin( TRI_QUADS );
	DrawQuad( (ScreenWidth / 2) - (ScreenHeight / 2), 0, (ScreenWidth / 2) + (ScreenHeight / 2), ScreenHeight );
	gEngfuncs.pTriAPI->End();
}
...
void CHudCrosshairStatic::DrawGaussZoomedHUD(void)
{
	...
	if( GaussWarning )
	{
		...
		{
			g_pParticles.SmokeParticles( (float *)&GET_VIEWMODEL()->attachment[0], 1, 0.5, 10 );
