...
int CHudHealth:: MsgFunc_Damage(const char *pszName,  int iSize, void *pbuf )
{
	...
	if( !g_iUser1 ) // not when in spectator
	{
		if( damageTaken > 25 )
			ApplyGaussBlur = true;
...
/*	
	...
	if (gHUD.HasWeapon( WEAPON_SUIT ))
	{
		...
		SPR_DrawAdditive( 0, x, y, &gHUD.GetSpriteRect( m_HUD_crocss ));
...
{	
	...
	for( int i = 0; i < NUM_DMG_TYPES; i++)
	{
		...
		if( bitsOn & giDmgFlags[i] )
		{
			...
			pdmg->x = giDmgWidth / 8;
			pdmg->y = ScreenHeight - giDmgHeight * 2 - 200; // diffusion - added 200 to put icons higher
