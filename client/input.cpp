...
kbutton_t	in_sprint;     //DiffusionSprint by Ku2zoff
...
void IN_SprintDown(void) {KeyDown(&in_sprint);}  // DiffusionSprint
...
void IN_DuckToggle( void ) { g_bDuckToggled = !g_bDuckToggled; }
...
/*	if( !( in_strafe.state & BUTTON_DOWN )) // diffusion - used for dash!
	{
...
{	
	...
	//		spd = gEngfuncs.GetClientMaxspeed();
			// diffusion - FIXME!!! "fix" of increased movement speed when moving diagonally
...
{
	// diffusion - gHUD.CanMove (for WASD) is done in CL_CreateMove. doesn't work here unlike jump and duck
	...
	if ( (in_sprint.state & (BUTTON_DOWN|IMPULSE_DOWN)) && gHUD.CanSprint ) // DiffusionSprint
	{
		bits |= IN_RUN;
	}
	...
	if( bResetState )
	{
		...
		in_sprint.state &= ~IMPULSE_DOWN; // DiffusionSprint
	}
...
void InitInput( void )
{
	...
	ADD_COMMAND( "duck_t", IN_DuckToggle );
	...
	ADD_COMMAND ("-sprint", IN_SprintUp); //DiffusionSprint
