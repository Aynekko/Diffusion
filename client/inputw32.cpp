...
POINT		current_pos;
float		mouse_x, mouse_y, old_mouse_x, old_mouse_y, mx_accum, my_accum; // diffusion - was int
...
void IN_MouseMove( float frametime, usercmd_t *cmd )
{
	...
	if( !gHUD.m_iIntermission && gHUD.CanMove )
	{
		...
		MouseSpeed = (Vector2D( mx, my ) - Vector2D( old_mouse_x, old_mouse_y )).Length();
		...
		if(( in_strafe.state & 1 ) || ( lookstrafe->value && ( in_mlook.state & 1 )))
			...
			viewangles[YAW] -= upside_down * m_yaw->value * mouse_x;
