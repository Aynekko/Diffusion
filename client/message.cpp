...
int CHudMessage::XPosition( float x, int width, int totalWidth )
{
	...
	{
		if( x < 0 )
			xPos = ( 1.0f + x ) * ScreenWidth - totalWidth;	// Align right
		else
			xPos = x * ScreenWidth;
	}
...
//						return;
//  diffusion - this was causing an issue: if we are calling the same sentence repeatedly,
//				it won't refresh, but rather fade out completely, and ONLY THEN it will appear again.
