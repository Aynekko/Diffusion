...
static void R_Bloom_InitEffectTexture( void )
{
	...
	{	// make sure bloom size is a power of 2
		for( BLOOM_SIZE = 16; (BLOOM_SIZE<<1) <= limit; BLOOM_SIZE <<= 1 );
	}
...
void R_BloomBlend( void )
{
	...
	{
		// diffusion - FIXME: I made those as 1 instead, because of a strange black line on 1280x1024 resolution,
		// maybe some other resolutions as well. It seems to work but I don't know why this is done here like this in the first place.
		sampleText_tcw = 1;// ((float)curView_width / (float)curView_height);
		sampleText_tch = 1.0f;
	}
	else
	{
		sampleText_tcw = 1.0f;
		sampleText_tch = 1;// ((float)curView_height / (float)curView_width);
	}
