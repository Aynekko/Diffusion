...
bool R_CullModel( cl_entity_t *e, const Vector &absmin, const Vector &absmax )
{
	...
	// never draw playermodel for his flashlight while shadowpass is active
	if( FBitSet( RI->params, RP_SHADOWPASS ) && RI->currentlight != NULL )
	{
	...
	if( RP_LOCALCLIENT( e ))
	{
		...
		if( !FBitSet( RI->params, RP_THIRDPERSON ) && UTIL_IsLocal( RI->viewentity ))
		{
			...
			if(!( RI->params & ( RP_MIRRORVIEW|RP_PORTALVIEW|RP_SCREENVIEW|RP_SHADOWPASS )))
				return true;
		}
	}
