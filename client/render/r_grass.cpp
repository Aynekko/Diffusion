...
{	
	if( pl->flags & CF_NOGRASSLIGHTING )
		return;
...
void R_GrassPrepareFrame( void )
{
	...
	{
		// grass in 3d sky is always visible
		m_flGrassFadeStart = 99999;
		m_flGrassFadeDist = 99999;
	}
	else
	{
		...
		m_flGrassFadeDist = Q_max( 0.0f, fabs(r_grass_fade_dist->value * (1 / FOVfactor)) );
	}
...
Fill polygon when player see it
================
*/
...
int R_GrassTextureForName( const char *name, byte *tex )
{
	...
	if( i == GRASS_TEXTURES )
	{
		ALERT( at_warning, "Limit of grass textures was exceeded (max %d)\n", GRASS_TEXTURES );
		return 1;
	}
...
void R_GrassInit( void )
{
	...
	char *afile = (char *)gEngfuncs.COM_LoadFile( "data/grassinfo.txt", 5, NULL );
...
void R_DrawGrass( qboolean lightpass )
{
	...
	qboolean shadowpass = FBitSet( RI->params, RP_SHADOWPASS );
