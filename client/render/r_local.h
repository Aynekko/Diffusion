...
#define RP_SHADOWPASS	BIT( 8 )	// view through light
...
#define RP_CUBEPASS()	( FBitSet( RI->params, RP_ENVVIEW ))
...
extern Vector PrevViewAngles;
extern Vector PrevViewOrg;
extern TEMPENTITY *pMuzzleflash[MAX_MUZZLEFLASH + 1];
...
typedef struct
{
	...
	unsigned short	gl_normalmap_id;			// normalmaps array

	float DynlightScale[MAX_LANDSCAPE_LAYERS]; // dynlight brightness scale
...
typedef struct
{
	...
	cl_entity_t *shader_modifier; // diffusion - entity which lets mapper to modify shader settings on the go

	float fadeblend[MAX_FADING_ENTS]; // diffusion - fading out
	float cableSwayIntensity[8192];
...
typedef struct
{
	...
	unsigned int	c_plights_meshes; // meshes in the light list
	...
	unsigned int	num_light_surfaces;

	unsigned int	sh_sunshafts_pass;
	unsigned int	sh_motionblur_pass;
	unsigned int	sh_gaussblur_pass;
	unsigned int	sh_horizontalblur_pass;
	unsigned int	sh_screenwater_pass;
	unsigned int	sh_monochrome_pass;
	unsigned int	sh_ssao_pass;
	unsigned int	sh_waterdrops_pass;
	unsigned int	sh_glitch_pass;
...
void ScreenToWorld( int mousex, int mousey, float fov, const Vector &vecRenderOrigin, const Vector &vecRenderAngles, Vector &vecPickingRay ); // diffusion - from Source SDK
...
void SetupMuzzlelightParams( const struct cl_entity_s *entity, Vector origin, int WeaponID );
