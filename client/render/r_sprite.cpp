...
float CSpriteModelRenderer :: GlowSightDistance( void )
{
	...
	if( !FBitSet( RI->params, RP_MIRRORVIEW ) || !FBitSet( RI->params, RP_SHADOWPASS ) )
	{
		//------------------------------------------
		int maxLoops = 4;
		int ignoreent = -1;	// first, ignore no entity
		Vector vecStart = RI->vieworg;
		while( maxLoops > 0 )
		{
			gEngfuncs.pEventAPI->EV_SetTraceHull( 2 );
			// diffusion - PM_GLASS_IGNORE and PM_TRACELINE_PHYSENTSONLY take a lot of fps...
			gEngfuncs.pEventAPI->EV_PlayerTrace( vecStart, sprite_origin, PM_TRACELINE_ANYVISIBLE, ignoreent, &tr );
			if( tr.ent > 0 )
			{
				cl_entity_t *ent = GET_ENTITY( PM_GetPhysEntInfo( tr.ent ) );
				if( ent != NULL )
				{
					if( ent->curstate.rendermode == kRenderNormal )
						return 0.0f;

					if( ent->curstate.rendermode == kRenderTransTexture && ent->curstate.renderamt == 0 ) // HACKHACK invisible brush "collision"
						return 0.0f;
			...
			ignoreent = tr.ent;	// ignore last hit entity
			vecStart = tr.endpos;
			maxLoops--;
		}
		//------------------------------------------
...
float CSpriteModelRenderer :: SpriteGlowBlend( int rendermode, int renderfx, float &scale )
{
	...
	if( rendermode != kRenderConstantGlow )
	{
...
void CSpriteModelRenderer :: SpriteDrawModel( void )
{
	...
	if( m_pCurrentEntity->model && Q_strstr( m_pRenderModel->name, "null" ) )
		return;
	...
	if( SpriteAllowLerping( m_pCurrentEntity, m_pSpriteHeader ))
		lerp = GetSpriteFrameInterpolant( &oldframe, &frame );
	else
		frame = oldframe = GetSpriteFrame( m_pCurrentEntity->curstate.frame, m_pCurrentEntity->angles[YAW] );

	int type;
	if( m_pCurrentEntity->curstate.iuser1 > 0 )
		type = m_pCurrentEntity->curstate.iuser1 - 1; // diffusion - use custom angle type
	else
		type = m_pSpriteHeader->type;
	...
	default:
		...
		Vector ViewVec;
		VectorAngles( m_pCurrentEntity->curstate.origin - RI->vieworg, ViewVec );
		gEngfuncs.pfnAngleVectors( ViewVec, NULL, v_right, v_up );
	#endif
		break;
	}
