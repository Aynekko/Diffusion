...
int R_SetupMirrorView( msurface_t *surf, cl_entity_t *camera, matrix4x4 &viewmatrix )
{
	...
	// diffusion - made quality settings
	float QualityLevel;

	switch( (int)r_mirrorquality->value )
	{
	case 1: QualityLevel = 0.15; break;
	case 2: QualityLevel = 0.35; break;
	case 3: QualityLevel = 0.7; break;
	default:
	case 4: QualityLevel = 1.0; break;
	}

	int ScreenW = glState.width * QualityLevel;
	int ScreenH = glState.height * QualityLevel;

	if( GL_Support( R_ARB_TEXTURE_NPOT_EXT ))
	{
		// allow screen size
		RI->viewport[2] = ScreenW;
		RI->viewport[3] = ScreenH;
		RI->viewport[2] = bound( 96, RI->viewport[2], glState.width );
		RI->viewport[3] = bound( 72, RI->viewport[3], glState.height );
		
	}
	else
	{
		RI->viewport[2] = NearestPOW( ScreenW, true );
		RI->viewport[3] = NearestPOW( ScreenH, true );
		RI->viewport[2] = bound( 128, RI->viewport[2], glState.width );
		RI->viewport[3] = bound( 64, RI->viewport[3], glState.height );
	}
...
int R_SetupPortalView( msurface_t *surf, cl_entity_t *camera, matrix4x4 &viewmatrix )
{
	...
	if( !R_StaticEntity( ent ))
	{
		if( ent->angles != g_vecZero )
			...
			surfaceMat = matrix4x4( ent->origin, g_vecZero );
	}
	else
		surfaceMat.Identity();
	...
	if( !R_StaticEntity( camera ))
	{
		if( camera->angles != g_vecZero )
			...
			cameraMat = matrix4x4( camera->origin, g_vecZero );
	}
	else
		cameraMat.Identity();
...
int R_SetupScreenView( msurface_t *surf, cl_entity_t *camera, matrix4x4 &viewmatrix )
{
 	...
 	if( GL_Support( R_ARB_TEXTURE_NPOT_EXT ))
	{
		...
		{
			RI->viewport[2] = bound( 64, t->width * 2, 2048 ); // diffusion - increased from 1024 to 2048
			RI->viewport[3] = bound( 64, t->height * 2, 2048 );
		}
	}
...
{
	if( !CVAR_TO_BOOL( r_mirrorquality ) )
		return; // disabled
...
void R_FindBmodelSubview( cl_entity_t *e )
{
	...
	for( int i = 0; i < clmodel->nummodelsurfaces; i++, psurf++ )
	{
		...
		if( FBitSet( psurf->flags, SURF_REFLECT ) && !CVAR_TO_BOOL( r_mirrorquality ))
			continue;

		if( FBitSet( psurf->flags, SURF_SCREEN ) && !CVAR_TO_BOOL( gl_allow_screens ))
			continue;

		if( FBitSet( psurf->flags, SURF_PORTAL ) && !CVAR_TO_BOOL( gl_allow_portals ))
			continue;
