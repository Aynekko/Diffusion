	...
	for( int i = 0; i < MAX_ICONSPRITES; i++ )
	{
		if( m_IconList[i].spr )
		{
			...
			if( IsAdditive == 1 )
				...
				SPR_Draw( 0, x, y, &m_IconList[i].rc );

	//		gEngfuncs.Con_NPrintf( 1, "x %i y %i\n", x, y );
		}
	}
	return 1;
}
...
void CHudStatusIcons::EnableIcon( const char *pszIconName, byte red, byte green, byte blue )
{
	...
	m_IconList[i].b = blue;
	Q_strncpy( m_IconList[i].szSpriteName, pszIconName, sizeof( m_IconList[i].szSpriteName ) );
}
