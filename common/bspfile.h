...
#define SURF_WATER_QUADS		BIT( 3 )		// all subidivided polygons are quads 
#define SURF_WATER		BIT( 4 )		// warp surface
...
#define LS_NONE			0xFF
#define LS_SUN			0x14
#define LS_SKY			0x15 // pxrad: -skystyle 21
...
#define MAX_MAP_LIGHTING		 0xf424000// diffusion - ~256 Mb      0x2000000		// 32 Mb lightmap raw data (can contain deluxemaps)
...
//	byte		deluxe[LM_STYLES][3];	// deluxe vectors
	color24 light[LM_STYLES];
	color24 deluxe[LM_STYLES];
	byte		shadow[LM_STYLES];		// shadowmap
} dvertlight_t;
/*
typedef struct // TTTEST
{
	color24		light[LM_STYLES];	// lightvalue
	color24		deluxe[LM_STYLES];	// deluxe vectors
	byte		shadow[LM_STYLES];	// shadowmap
} dvertlight_t;*/
