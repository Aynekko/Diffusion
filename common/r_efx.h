...
// TEMPENTITY flags
#define FTENT_NONE			0
#define FTENT_SINEWAVE		BIT(0)
#define FTENT_GRAVITY		BIT(1)
#define FTENT_ROTATE		BIT(2)
#define FTENT_SLOWGRAVITY	BIT(3)
#define FTENT_SMOKETRAIL	BIT(4)
#define FTENT_COLLIDEWORLD	BIT(5)
#define FTENT_FLICKER		BIT(6)
#define FTENT_FADEOUT		BIT(7)
#define FTENT_SPRANIMATE	BIT(8)
#define FTENT_HITSOUND		BIT(9)
#define FTENT_SPIRAL		BIT(10)
#define FTENT_SPRCYCLE		BIT(11)
#define FTENT_COLLIDEALL	BIT(12) // will collide with world and slideboxes
#define FTENT_PERSIST		BIT(13) // tent is not removed when unable to draw 
#define FTENT_COLLIDEKILL	BIT(14) // tent is removed upon collision with anything
#define FTENT_PLYRATTACHMENT BIT(15) // tent is attached to a player (owner)
#define FTENT_SPRANIMATELOOP BIT(16) // animating sprite doesn't die when last frame is displayed
#define FTENT_SPARKSHOWER	BIT(17)
#define FTENT_NOMODEL		BIT(18) // Doesn't have a model, never try to draw ( it just triggers other things )
