...
typedef int SpriteHandle;	// handle to a graphic
...
{
	// sprite handlers
	SpriteHandle	(*pfnSPR_Load)( const char *szPicName );
	int	(*pfnSPR_Frames)( SpriteHandle hPic );
	int	(*pfnSPR_Height)( SpriteHandle hPic, int frame );
	int	(*pfnSPR_Width)( SpriteHandle hPic, int frame );
	void	(*pfnSPR_Set)( SpriteHandle hPic, int r, int g, int b );
	...
	void (*pfnPlaybackEvent)(int flags, const struct edict_s *pInvoker, unsigned short eventindex, float delay, const float *origin, const float *angles, float fparam1, float fparam2, int iparam1, int iparam2, int bparam1, int bparam2);	void	(*pfnWeaponAnim)( int iAnim, int body );
