...
public:
	MemBlock(int numElements)
	{
		...
		m_iSIZEOFARRAY = numElements + 1;
		m_pArray = new chunk_t[m_iSIZEOFARRAY];		

		if (!m_pArray)
		{
			m_iSIZEOFARRAY = 1;
			m_iFirstFree = m_iSIZEOFARRAY;
			return;
		}
	...
	void Clear( void )
	{
		if (m_iSIZEOFARRAY > 1)
		{
			...
			for (int i = 1; i < m_iSIZEOFARRAY; ++i)
				m_pArray[i].next = i + 1;
		}
	}
	...
	T* Allocate( void )
	{
		if (m_iFirstFree != m_iSIZEOFARRAY)
		{
