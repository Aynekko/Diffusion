...
void PM_PlayStepSound( int step, float fvol )
{
	...
	{
		// diffusionFIXME g_onladder is always 0?
//		if( !g_onladder && hvel.Length() <= 200 )
		if( (step != STEP_LADDER) && hvel.Length() <= 150 )
			return;
	}
	...
	*/
	...
	SplashOrg[2] = pmove->origin[2] - 0.3 * height;
	TEMPENTITY *pTemp;
	int iWaterSplash;
#endif
	...
	case STEP_WADE:
		...
		*/
		...
		// UNDONE - this must be unified in a generic watersplash function, for now I copy-pasted it from hud_msg
#ifdef CLIENT_DLL
		iWaterSplash = gEngfuncs.pEventAPI->EV_FindModelIndex( "sprites/effects/waterring2.spr" );
...
//		wishvel[i] = pmove->forward[i]*fmove + pmove->right[i]*smove;
	for (i=0 ; i<2 ; i++)       // Determine x and y parts of velocity DiffusionSprint
	{
		if (bIsSprint && (!( pmove->flags & FL_DUCKING)))
			wishvel[i] = pmove->forward[i]*(fmove * iSprintFactor) + pmove->right[i]*smove;
		else
			wishvel[i] = pmove->forward[i]*fmove + pmove->right[i]*smove;
	}
...
*/
	...
	case MOVETYPE_WALK:
		...
		{
			// Was jump button pressed?
			if ( (pmove->cmd.buttons & IN_JUMP)/* && !(pmove->flags2 & FL2_PLAYER_NOJUMP)*/)
			{
				if ( !pLadder )
					PM_Jump ();
			}
