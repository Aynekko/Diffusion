...
uniform sampler2D		u_NormalMap; // distortion map
...
void main( void )
{
	...
	vec2 distortionVec = texture2D( u_NormalMap, fract( Speed * 0.05 * uv.xy + u_RealTime * vec2( 0.0, 0.125 ))).rg;
	...
	if( bool( u_Speed == 0.5f )) // underwater
	{
		outputColor *= 0.75;
	}
