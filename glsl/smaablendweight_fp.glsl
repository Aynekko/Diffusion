...
uniform sampler2D u_ColorMap; // edges
uniform sampler2D u_AreaTex; // area
uniform sampler2D u_SearchTex; // search
...
vec2 SMAASearchDiag1( sampler2D edgesTex, vec2 texcoord, vec2 dir, out vec2 e )
{
		...
		coord.w > 0.9 )
	{
		...
		e = texture2DLod( edgesTex, coord.xy, 0.0 ).rg; // LinearSampler
...
vec2 SMAAAreaDiag( sampler2D areaTex, vec2 dist, vec2 e, float offset )
{
	...
	// Do it!
	return texture2DLod( areaTex, texcoord, 0.0 ).ra; // LinearSampler
}
...
vec2 SMAACalculateDiagWeights( sampler2D edgesTex, sampler2D areaTex, vec2 texcoord, vec2 e, vec4 subsampleIndices )
{
	...
	if( d.x + d.y > 2.0 ) // d.x + d.y + 1 > 3
	{
		...
		vec4 coords = mad( vec4( -d.x + 0.25, API_V_DIR( d.x ), d.y, API_V_DIR( -d.y - 0.25 ) ), SMAA_RT_METRICS.xyxy, texcoord.xyxy );
		vec4 c;
	...
	{
		// Fetch the crossing edges:
		vec4 coords = mad( vec4( -d.x, API_V_DIR( -d.x ), d.y, API_V_DIR( d.y ) ), SMAA_RT_METRICS.xyxy, texcoord.xyxy );
		vec4 c;
...
float SMAASearchLength( sampler2D searchTex, vec2 e, float offset )
{
	...
	return texture2DLod( searchTex, coord, 0.0 ).r; // LinearSampler
}
...
float SMAASearchXLeft( sampler2D edgesTex, sampler2D searchTex, vec2 texcoord, float end )
{
	...
	*/
	...
	{
		e = texture2DLod( edgesTex, texcoord, 0.0 ).rg; // LinearSampler
...
float SMAASearchYUp( sampler2D edgesTex, sampler2D searchTex, vec2 texcoord, float end )
{
	vec2 e = vec2( 1.0, 0.0 );
	//for (int i = 0; i < SMAA_MAX_SEARCH_STEPS; i++) { 
	//if (!(API_V_BELOW(texcoord.y, end) && e.r > 0.8281 && e.g == 0.0)) break;
	while( API_V_BELOW( texcoord.y, end ) &&
		e.r > 0.8281 && // Is there some edge not activated?
		e.g == 0.0 ) // Or is there a crossing edge that breaks the line?
	{
		e = texture2DLod( edgesTex, texcoord, 0.0 ).rg; // LinearSampler
		texcoord = mad( -vec2( 0.0, API_V_DIR( 2.0 ) ), SMAA_RT_METRICS.xy, texcoord );
	}
...
float SMAASearchYDown( sampler2D edgesTex, sampler2D searchTex, vec2 texcoord, float end )
{
	vec2 e = vec2( 1.0, 0.0 );
	//for (int i = 0; i < SMAA_MAX_SEARCH_STEPS; i++) { 
	//if (!(API_V_ABOVE(texcoord.y, end) && e.r > 0.8281 && e.g == 0.0)) break;
...
void main( void )
{
	...
	vec2 e = texture2D( u_ColorMap, var_TexCoord ).rg;
	...
	if( e.g > 0.0 ) // Edge at north
	{
		...
		weights.rg = SMAACalculateDiagWeights( u_ColorMap, u_AreaTex, var_TexCoord, e, subsampleIndices );
		...
		if( weights.r == -weights.g ) { // weights.r + weights.g == 0.0
			...
			coords.x = SMAASearchXLeft( u_ColorMap, u_SearchTex, var_Offset[0].xy, var_Offset[2].x );
			coords.y = var_Offset[1].y; // var_Offset[1].y = var_TexCoord.y - 0.25 * SMAA_RT_METRICS.y (@CROSSING_OFFSET)
			d.x = coords.x;

			// Now fetch the left crossing edges, two at a time using bilinear
			// filtering. Sampling at -0.25 (see @CROSSING_OFFSET) enables to
			// discern what value each edge has:
			e1 = texture2DLod( u_ColorMap, coords.xy, 0.0 ).r; // LinearSampler

			// Find the distance to the right:
			coords.z = SMAASearchXRight( u_ColorMap, u_SearchTex, var_Offset[0].zw, var_Offset[2].y );
			d.y = coords.z;

			// We want the distances to be in pixel units (doing this here allow to
			// better interleave arithmetic and memory accesses):
			d = abs( SMAARound( mad( SMAA_RT_METRICS.zz, d, -var_PixCoord.xx ) ) );
			...
			e2 = SMAASampleLevelZeroOffset( u_ColorMap, coords.zy, ivec2( 1, 0 ) ).r;

			// Ok, we know how this pattern looks like, now it is time for getting
			// the actual area:
			weights.rg = SMAAArea( u_AreaTex, sqrt_d, e1, e2, subsampleIndices.y );

			// Fix corners:
			coords.y = var_TexCoord.y;
			SMAADetectHorizontalCornerPattern( u_ColorMap, weights.rg, coords.xyzy, d );
	...
	if( e.r > 0.0 ) // Edge at west
	{
		...
		coords.y = SMAASearchYUp( u_ColorMap, u_SearchTex, var_Offset[1].xy, var_Offset[2].z );
		coords.x = var_Offset[0].x; // var_Offset[1].x = var_TexCoord.x - 0.25 * SMAA_RT_METRICS.x;
		d.x = coords.y;

		// Fetch the top crossing edges:
		e1 = texture2DLod( u_ColorMap, coords.xy, 0.0 ).g; // LinearSampler

		// Find the distance to the bottom:
		coords.z = SMAASearchYDown( u_ColorMap, u_SearchTex, var_Offset[1].zw, var_Offset[2].w );
		d.y = coords.z;

		// We want the distances to be in pixel units:
		d = abs( SMAARound( mad( SMAA_RT_METRICS.ww, d, -var_PixCoord.yy ) ) );
		...
		e2 = SMAASampleLevelZeroOffset( u_ColorMap, coords.xz, ivec2( 0, API_V_DIR( 1 ) ) ).g;

		// Get the area for this direction:
		weights.ba = SMAAArea( u_AreaTex, sqrt_d, e1, e2, subsampleIndices.x );

		// Fix corners:
		coords.x = var_TexCoord.x;
		SMAADetectVerticalCornerPattern( u_ColorMap, weights.ba, coords.xyxz, d );
	}
