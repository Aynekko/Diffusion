...
float FakeSpecMap( vec3 ColorMap )
{
    return clamp( dot( ColorMap, vec3( 0.33 ) ), 0.0, 1.0 );
}

vec3 DiffuseToGlossmap( sampler2D ColorMap, vec2 texcoord )
{
    vec3 res = texture2D( ColorMap, texcoord ).rgb;
    vec3 glossmap = vec3( FakeSpecMap( res ) );
    glossmap *= glossmap;
...
vec3 DiffuseToGlossmapTerrain( sampler2DArray ColorMap, vec2 texcoord, vec4 mask0, vec4 mask1, vec4 mask2, vec4 mask3 )
{
    ...
    glossmap = vec3( FakeSpecMap( res.rgb ) );
#endif
...
{
    float roughness = max( SmoothnessToRoughness( smoothness ), 0.001 );  // Prevent highlights from getting too tiny without area lights
