...
void CRC32_ProcessByte( uint32_t *pulCRC, uint8_t ch )
{
	...
	ulCrc ^= ch;
	ulCrc = crc32table[(uint8_t)ulCrc] ^ (ulCrc >> 8);
	*pulCRC = ulCrc;
}
...
JustAfew:
	switch( nBuffer )
	{
	case 7: ulCrc  = crc32table[*pb++ ^ (uint8_t)ulCrc] ^ (ulCrc >> 8); // fallthrough
	case 6: ulCrc  = crc32table[*pb++ ^ (uint8_t)ulCrc] ^ (ulCrc >> 8); // fallthrough
	case 5: ulCrc  = crc32table[*pb++ ^ (uint8_t)ulCrc] ^ (ulCrc >> 8); // fallthrough
	case 4:
		memcpy( &tmp, pb, sizeof(uint32_t));
		...
		*pulCRC = ulCrc;
		return;
	case 3: ulCrc  = crc32table[*pb++ ^ (uint8_t)ulCrc] ^ (ulCrc >> 8); // fallthrough
	case 2: ulCrc  = crc32table[*pb++ ^ (uint8_t)ulCrc] ^ (ulCrc >> 8); // fallthrough
	case 1: ulCrc  = crc32table[*pb++ ^ (uint8_t)ulCrc] ^ (ulCrc >> 8); // fallthrough
	case 0: *pulCRC = ulCrc;
		return;
	}
	...
	while( nMain-- )
	{
		...
		ulCrc  = crc32table[(uint8_t)ulCrc] ^ (ulCrc >> 8);
		ulCrc  = crc32table[(uint8_t)ulCrc] ^ (ulCrc >> 8);
		memcpy( &tmp, pb + 4, sizeof(uint32_t));
		ulCrc ^= tmp; // warning, this only works on little-endian.
		ulCrc  = crc32table[(uint8_t)ulCrc] ^ (ulCrc >> 8);
		ulCrc  = crc32table[(uint8_t)ulCrc] ^ (ulCrc >> 8);
		ulCrc  = crc32table[(uint8_t)ulCrc] ^ (ulCrc >> 8);
