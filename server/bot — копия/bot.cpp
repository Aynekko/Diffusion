...
void CBot::Spawn( )
{
	...
	bot_model = 0;
	if ((strcmp( model_name, "hgrunt" ) == 0) || (strcmp( model_name, "recon" ) == 0))
	{
	...
	LastKnownWaypoint = g_vecZero;
	iNearbyWaypointNum = 0;
	PreviousWaypoint = g_vecZero;
...
int CBot::TakeDamage( entvars_t *pevInflictor, entvars_t *pevAttacker, float flDamage, int bitsDamageType )
{
	...
	if ((bot_model != 0) && (pAttacker->IsNetClient()) && (f_pain_time <= gpGlobals->time) && (pev->health > 0) && ( !IS_DEDICATED_SERVER() ))
	{
		...
		if (distance <= 400)
		{
			...
			if (RANDOM_LONG(1, 100) <= 50)
			{
				...
				EMIT_SOUND(ENT(pevAttacker), CHAN_VOICE, sound, RANDOM_FLOAT(0.9, 1.0), ATTN_NORM);
			}
		}
	}
				...
				USE_TYPE useType, float value )
{
	if (g_pGameRules->IsTeamplay())  // is team play enabled?
	{
		...
		if (UTIL_TeamsMatch(g_pGameRules->GetTeamID(this), g_pGameRules->GetTeamID(pActivator)))
		{
...
BOOL CBot::BotEntityIsVisible( Vector dest )
{
	...
	UTIL_TraceLine( pev->origin + pev->view_ofs, dest, ignore_monsters, ENT(pev), &tr );
...
bool CBot::BotTryWaypoint( void )
{
	...
	if( LastKnownWaypoint != g_vecZero )
	{
		if( (pev->origin - LastKnownWaypoint).Length() > 50 ) // check last received point, we are still going there
		{
			UTIL_TraceLine( pev->origin + pev->view_ofs, LastKnownWaypoint, ignore_monsters, ENT( pev ), &tr );

			// visible!
			if( tr.flFraction == 1.0 )
			{
				f_cantseelastwaypoint_time = 0;
				
				// let's head off toward that point...
				Vector vPoint = LastKnownWaypoint - pev->origin;
				...
				{
					if( m_flStaminaValue > 50 && gpGlobals->time > last_dash_time + 7  ) // try dash
					{
						pev->button |= IN_FORWARD;
						pev->button |= IN_LEFT;
						last_dash_time = gpGlobals->time;
					}
				}
			...
			{
				// I can't see the waypoint but try to go there for 3 more seconds
				if( f_cantseelastwaypoint_time == 0.0f )
					f_cantseelastwaypoint_time = gpGlobals->time + 3;
				
				if( gpGlobals->time > f_cantseelastwaypoint_time )
					LastKnownWaypoint = g_vecZero; // not valid anymore
			}
		}
		else
		{
			LastKnownWaypoint = g_vecZero; // we are too close to this point, invalidate it and choose another
		}
	}

	if( LastKnownWaypoint == g_vecZero && gpGlobals->time > f_trynextwaypoint_time ) // search for a new point every now and then
	{
		FindNearbyWaypoints( 1000 );
		...
		for( int i = 0; i < iNearbyWaypoints; i++ )
		{
			...
			if( (vNearbyWaypoints[i] - PreviousWaypoint).Length() < 250 )
				continue; // I've just been there
			
			// only interested in far enough points
			if( (pev->origin - vNearbyWaypoints[i]).Length() >= 100 )
			{
	...
	{
		// let's head off toward that point...
		Vector vPoint = vNearbyWaypoints[iNearbyWaypointNum] - pev->origin;
	 ...
	 //   ALERT( at_aiconsole, "Bot %i going to new waypoint (%i out of %i)\n", ENTINDEX( edict() ), iNearbyWaypointNum, iNearbyWaypoints );
		ALERT( at_aiconsole, "PreviousWaypoint %.f %.f %.f, chosen %.f %.f %.f\n", PreviousWaypoint.x, PreviousWaypoint.y, PreviousWaypoint.z, vNearbyWaypoints[iNearbyWaypointNum].x, vNearbyWaypoints[iNearbyWaypointNum].y, vNearbyWaypoints[iNearbyWaypointNum].z );

		LastKnownWaypoint = vNearbyWaypoints[iNearbyWaypointNum];

		PreviousWaypoint = pev->origin; // record rough position of our starting point

	//    UTIL_Sparks( vNearbyWaypoints[iNearbyWaypointNum] );
...
bool CBot::BotTryWaypoint(void)
{
	...
	bool WaypointFound = false;
	int WaypointNumber = 0;
	int Checks = 15; // check only 15 times to avoid loops

	if( last_waypoint_num > 0 && (pev->origin - vWaypoint[last_waypoint_num]).Length() > 300 ) // check last received point, we are still going there
	{
		UTIL_TraceLine( pev->origin, vWaypoint[last_waypoint_num], ignore_monsters, ENT( pev ), &tr );

		// visible!
		if( tr.flFraction == 1.0 )
		{
			WaypointFound = true;
			WaypointNumber = last_waypoint_num;
		}
	}

	if( !WaypointFound && gpGlobals->time > f_trynextwaypoint_time ) // search for a new point every now and then
	{
		while( !WaypointFound && Checks > 0 )
		{
			WaypointNumber = RANDOM_LONG( 0, WaypointsLoaded-1 );
			// trace a line from bot's ORIGIN to destination...
			UTIL_TraceLine( pev->origin, vWaypoint[WaypointNumber], ignore_monsters, ENT( pev ), &tr );

			// visible!
			if( tr.flFraction == 1.0 )
			{
				// not allowed if too far
				if( (pev->origin - vWaypoint[WaypointNumber]).Length() < 3000 )
					WaypointFound = true;
			}
		...
		f_trynextwaypoint_time = gpGlobals->time + RANDOM_FLOAT(0.75,2.0);
	}

	if( WaypointFound )
	{
		// let's head off toward that item...
		Vector vPoint = vWaypoint[WaypointNumber] - pev->origin;
		...
		if( WaypointNumber != last_waypoint_num )
			ALERT( at_aiconsole, "Bot %i going to waypoint %i\n", ENTINDEX( edict() ), WaypointNumber );

		last_waypoint_num = WaypointNumber;

		UTIL_Sparks( vWaypoint[WaypointNumber] );
