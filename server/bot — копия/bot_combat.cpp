...
ammo_check_t ammo_check[] = {
   ...
   {"ammo_ar2balls", "ar2balls", M203_GRENADE_MAX_CARRY},
   {"ammo_fiveseven", "ammo57", 100 },
...
// BigGuy - END
   ...
   if( (HasWeapon( WEAPON_FIVESEVEN )) && (pev->waterlevel != 3) )
   {
           ...
           (weapon_choice == WEAPON_FIVESEVEN) )
       {
           ...
           if( primary_ammo[WEAPON_FIVESEVEN] > 0 )
           {
               // check if the bot isn't already using this item...
               if( m_pActiveItem != new_weapon )
                   SelectItem( "weapon_fiveseven" );  // select the 357 python
   ...
   if ((HasWeapon(WEAPON_MRC)) && (pev->waterlevel != 3))
   {
      ...
      if (((distance > 300) && (distance < 1000) &&
           (weapon_choice == 0) && (use_secondary <= 15)) ||
          ((weapon_choice == WEAPON_MRC) && (primary == FALSE)))
      {
...
// BigGuy - START
         ...
         if (secondary_ammo[WEAPON_MRC] > 0)
         {
            ...
            // set next time to shoot
            f_shoot_time = gpGlobals->time + 1.0 +
               RANDOM_FLOAT(secondary_fire_delay[WEAPON_MRC][bot_skill][0],
                            secondary_fire_delay[WEAPON_MRC][bot_skill][1]);
...
// BigGuy - END
          ...
          (weapon_choice == WEAPON_MRC))
      {
         ...
         if (primary_ammo[WEAPON_MRC] > 0)
         {
            ...
            // set next time to shoot
            f_shoot_time = gpGlobals->time + 0.1 +
               RANDOM_FLOAT(primary_fire_delay[WEAPON_MRC][bot_skill][0],
                            primary_fire_delay[WEAPON_MRC][bot_skill][1]);
   ...
   if( (HasWeapon( WEAPON_AR2 )) && (pev->waterlevel != 3) )
   {
       ...
       if( ((distance > 300) && (distance < 1500) &&
           (weapon_choice == 0) && (use_secondary <= 15)) ||
           ((weapon_choice == WEAPON_AR2) && (primary == FALSE)) )
       {
           // at some point we need to fire upwards in the air slightly
           // for long distance kills.  for right now, just fire the
           // grenade at the poor sucker.
  ...
  // BigGuy - START
           ...
           if( secondary_ammo[WEAPON_AR2] > 0 )
           {
               // check if the bot isn't already using this item...
               if( m_pActiveItem != new_weapon )
                   SelectItem( "weapon_ar2" );  // select the 9mmAR (MP5)

               pev->button |= IN_ATTACK2;  // use secodnary attack (boom!)

               // set next time to shoot
               f_shoot_time = gpGlobals->time + 1.0 +
                   RANDOM_FLOAT( secondary_fire_delay[WEAPON_AR2][bot_skill][0],
                       secondary_fire_delay[WEAPON_AR2][bot_skill][1] );
           ...
           (weapon_choice == WEAPON_AR2) )
       {
           ...
           if( primary_ammo[WEAPON_AR2] > 0 )
           {
               ...
               // set next time to shoot
               f_shoot_time = gpGlobals->time + 0.1 +
                   RANDOM_FLOAT( primary_fire_delay[WEAPON_AR2][bot_skill][0],
                       primary_fire_delay[WEAPON_AR2][bot_skill][1] );
   ...
   if( (HasWeapon( WEAPON_HKMP5 )) && (pev->waterlevel != 3) )
   {
           ...
           (weapon_choice == WEAPON_HKMP5) )
       {
           ...
           if( primary_ammo[WEAPON_HKMP5] > 0 )
           {
               // check if the bot isn't already using this item...
               if( m_pActiveItem != new_weapon )
                   SelectItem( "weapon_hkmp5" );  // select the 357 python

               pev->button |= IN_ATTACK;  // use primary attack (bang! bang!)

               // set next time to shoot
               f_shoot_time = gpGlobals->time + 0.75 +
                   RANDOM_FLOAT( primary_fire_delay[WEAPON_HKMP5][bot_skill][0],
                       primary_fire_delay[WEAPON_HKMP5][bot_skill][1] );
   ...
   {
       if( ((distance < 1300) && (weapon_choice == 0)) ||
           (weapon_choice == WEAPON_G36C) )
       {
           ...
           if( primary_ammo[WEAPON_G36C] > 0 )
           {
               // check if the bot isn't already using this item...
               if( m_pActiveItem != new_weapon )
                   SelectItem( "weapon_g36c" );  // select the 357 python

               pev->button |= IN_ATTACK;  // use primary attack (bang! bang!)

               // set next time to shoot
               f_shoot_time = gpGlobals->time + 0.75 +
                   RANDOM_FLOAT( primary_fire_delay[WEAPON_G36C][bot_skill][0],
                       primary_fire_delay[WEAPON_G36C][bot_skill][1] );
   ...
   if (HasWeapon(WEAPON_CROSSBOW))
   {
      ...
      if (((distance > 250) && (distance < 1500) && (weapon_choice == 0)) ||
   ...
   {
       // if bot is not too close for crossbow and not too far...
       if( ((distance > 300) && (distance < 3000) && (weapon_choice == 0)) ||
           (weapon_choice == WEAPON_SNIPER) )
       {
           ...
           if( primary_ammo[WEAPON_SNIPER] > 0 )
           {
               ...
               // set next time to shoot
               f_shoot_time = gpGlobals->time + 0.75 +
                   RANDOM_FLOAT( primary_fire_delay[WEAPON_SNIPER][bot_skill][0],
                       primary_fire_delay[WEAPON_SNIPER][bot_skill][1] );
