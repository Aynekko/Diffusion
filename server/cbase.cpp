...
void CBaseEntity :: CheckAngles( void )
{
	...
	{
		ALERT( at_error, "Entity %s [%i] with model %s missed 'origin' brush. Entity removed!\n", GetClassname(), entindex(), GetModel() );
		// diffusion - show error and delete
		UTIL_Remove( this );
		return;
	}
...
void CBaseEntity :: SetParent( CBaseEntity *pParent, int m_iAttachment )
{
	...
	// make sure that child is the point entity and the parent have studiomodel
...
{
	// diffusion - this flag sets after calling UpdateOnRemove, but this function was being called more than once.
	// this is a check to prevent lots of calls
	...
	// diffusion - clear the beams, sprites, sounds of the entity
	ClearEffects();
...
BEGIN_DATADESC_NO_BASE( CBaseEntity )
	...
	DEFINE_KEYFIELD( CustomNameString, FIELD_STRING, "customname" ),
	...
	DEFINE_FIELD( EnableHealthBar, FIELD_BOOLEAN),
	DEFINE_FIELD( HealthBarType, FIELD_INTEGER),
	DEFINE_FIELD( bForceHealthbar, FIELD_BOOLEAN ),
	DEFINE_FIELD( CanTakeEMPDamage, FIELD_BOOLEAN ),
...
BOOL CBaseEntity :: IsInWorld( BOOL checkVelocity )
{
	...
	int WorldBounds = 262000; // FIXME

	// position 
	if( absOrigin.x >= WorldBounds ) return FALSE;
	if( absOrigin.y >= WorldBounds ) return FALSE;
	if( absOrigin.z >= WorldBounds ) return FALSE;
	if( absOrigin.x <= -WorldBounds ) return FALSE;
	if( absOrigin.y <= -WorldBounds ) return FALSE;
	if( absOrigin.z <= -WorldBounds ) return FALSE;
...
*/ 
	...
	float absVelocity = GetAbsVelocity().Length();
	float limitVelocity = CVAR_GET_FLOAT( "sv_maxvelocity" );
	if( limitVelocity == 0 )
		limitVelocity = 10000;
	if( fabs(absVelocity) > limitVelocity )
		return FALSE;
