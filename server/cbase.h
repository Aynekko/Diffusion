...
public:
	...
	int		m_iFlags2;		// diffusion - I'll put my stuff over here, don't mind me :)

	int m_iCounter; // diffusion - for mods
	...
	string_t CustomNameString; // diffusion - used to show an npc name when you look at him.
	...
	virtual void	KeyValue( KeyValueData *pkvd )
	{
		...
		{
			CustomNameString = ALLOC_STRING( pkvd->szValue );
			pkvd->fHandled = TRUE;
		}
	...
	virtual void AddSpawnFlags(int nFlags);
	virtual bool HasSpawnFlags(int nFlags) const;
	virtual void SetFadeDistance(int Distance);
	// m_iFlags2 for diffusion
	virtual void SetFlag( int Flag );
	virtual bool HasFlag( int Flag ) const;
	virtual void RemoveFlag( int Flag );
	virtual void SetNullModel( void );

	// for holdable objects, called by player on pickup/drop
	...
	bool CheckingLocalMove; // diffusion - monster is checking his movement, so ignore his origin while that happens
...
#define bits_CAP_DUCK		BIT(0) // crouch
#define bits_CAP_JUMP		BIT( 1 ) // jump/leap
#define bits_CAP_STRAFE		BIT(2 ) // strafe ( walk/run sideways)
#define bits_CAP_SQUAD		BIT( 3 ) // can form squads
#define bits_CAP_SWIM		BIT( 4 ) // proficiently navigate in water
#define bits_CAP_CLIMB		BIT( 5 ) // climb ladders/ropes
#define bits_CAP_USE		BIT( 6 ) // open doors/push buttons/pull levers
#define bits_CAP_HEAR		BIT( 7 ) // can hear forced sounds
#define bits_CAP_AUTO_DOORS		BIT( 8 ) // can trigger auto doors
#define bits_CAP_OPEN_DOORS		BIT( 9 ) // can open manual doors
#define bits_CAP_TURN_HEAD		BIT(10) // can turn head, always bone controller 0
#define bits_CAP_RANGE_ATTACK1	BIT( 11) // can do a range attack 1
#define bits_CAP_RANGE_ATTACK2	BIT( 12) // can do a range attack 2
#define bits_CAP_MELEE_ATTACK1	BIT( 13) // can do a melee attack 1
#define bits_CAP_MELEE_ATTACK2	BIT( 14) // can do a melee attack 2
#define bits_CAP_FLY		BIT( 15) // can fly, move all around
#define bits_CAP_CANSEEFLASHLIGHT BIT( 16) // can see flashlight (sets for all monsters initially - but drones and turrets can't recognize it)
...
#define TRACER_FREQ		0//4			// Tracers fire every 4 bullets
...
// fadedistance must be set as last of all properties( baseline issues - see R_ComputeFadingDistance )
inline void CBaseEntity::SetFadeDistance(int Distance)
{
	pev->iuser4 = Distance;
}

inline bool CBaseEntity::HasFlag(int Flag) const
{
	return (m_iFlags2 & Flag) != 0;
}

inline void CBaseEntity::SetFlag(int Flag)
{
	m_iFlags2 |= Flag;
}

inline void CBaseEntity::RemoveFlag( int Flag )
{
	m_iFlags2 &= ~Flag;
}
