...
#define SF_BEAM_STARTPOINT	BIT(10) // use env_beam itself as a start point
...
#define SF_FLARE_THROW		BIT(0) // when used by a trigger, it will be thrown in "angles" direction with desired velocity
#define SF_FLARE_DONTHURT	BIT(1) // do not hurt anyone until player picks it up at least once

class CFlare : public CBaseMonster
{
	DECLARE_CLASS( CFlare, CBaseMonster );
public:
	void Spawn( void );
	void Precache( void );
	...
	float m_flTimeStartBurn;
	float m_flTimeBurnOut;
	bool EffectsCreated;

	float m_flPitchChangeTime;
	int CurPitch;

//	int LightRadius; // pev->iuser2
	bool StaticFlareOn;
...
#define SF_PROJECTOR_MOVIESOUND BIT(10)
