...
void CChangeLevel::ChangeLevelNow(CBaseEntity* pActivator)
{
	...
	// diffusion - HACKHACK reset drone health and ammo, remove all active drones and compensate the player with a drone
	CBasePlayer* pPlayerx = (CBasePlayer*)GET_PRIVATE(INDEXENT(1));
	if( pPlayerx )
	{
		...
		if( pPlayerx->m_pFlashlightMonster )
		{
			UTIL_Remove( pPlayerx->m_pFlashlightMonster );
