...
void CGameText::KeyValue( KeyValueData *pkvd )
{
	...
	else if (FStrEq(pkvd->szKeyName, "holdtime"))
	{
		...
		// diffusion - it seems 2 mins is the limit. anything above just doesn't go through (text disappears fast or doesn't appear at all)
		m_textParms.holdTime = bound(0, m_textParms.holdTime, 120);
		pkvd->fHandled = TRUE;
	}
...
#define SF_PLAYEREQUIP_USEONLY	BIT(0)
#define MAX_EQUIP		32
	...
	for ( int i = 0; i < MAX_EQUIP; i++ )
	{
		...
		for ( int j = 0; j < m_weaponCount[i]; j++ )
		{
			...
			if( pEnt )
			{
				...
				// this is to prevent the ammo/weapon spawn abuse, i.e. from ammo crates
				pEnt->SetFlag(F_FROM_AMMOBOX);
				DispatchTouch( pEnt->edict(), pPlayer->edict() );
			}
			else
			{
				ALERT( at_error, "game_player_equip \"%s\" tried to spawn an unknown entity \"%s\"!\n", STRING( pev->targetname ), (char *)STRING( m_weaponNames[i] ) );
				break;
			}
		}
	}
