...
BEGIN_DATADESC( CRope )
	DEFINE_KEYFIELD( m_iSegments, FIELD_INTEGER, "segments" ),
	...
	DEFINE_KEYFIELD( m_bDisallowPlayerAttachment, FIELD_BOOLEAN, "disable" ),
	DEFINE_KEYFIELD( m_iszBodyModel, FIELD_STRING, "bodymodel" ),
	DEFINE_KEYFIELD( m_iszEndingModel, FIELD_STRING, "endingmodel" ),
...
void CRope :: SendUpdateBones( void )
{
	...
	studiohdr_t *phdr = (studiohdr_t *)CBaseAnimating::GetModelPtr( pev->modelindex ); //diffusionanimfix
	if( !phdr ) return;
	...
	MESSAGE_BEGIN( MSG_PVS, gmsgSetupBones, pev->origin );
		...
		WRITE_BYTE( phdr->numbones );                        //diffusionanimfix
...
void CRope :: Think( void )
{
	...
	if( ShouldCreak() )
	{
		EMIT_SOUND( edict(), CHAN_BODY, g_pszCreakSounds[RANDOM_LONG( 0, SIZEOFARRAY( g_pszCreakSounds ) - 1 )], VOL_NORM, ATTN_NORM );
	}
