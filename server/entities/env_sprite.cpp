...
// diffusion - iuser1 will override the angle type of the sprite! see SpriteDrawModel in r_sprite.cpp
...
void CSprite::Spawn(void)
{
	...
	if (pev->targetname && !HasSpawnFlags(SF_SPRITE_STARTON))
		TurnOff();
	else
		TurnOn();
	...
	if( pev->iuser1 > 0 )
		...
		type = UTIL_GetSpriteType( pev->modelindex );

	// Worldcraft only sets y rotation, copy to Z
	if (type != SPR_ORIENTED && GetLocalAngles().y != 0 && GetLocalAngles().z == 0)
	{
...
void CSprite::Animate(float frames)
{
	...
	{
		if (HasSpawnFlags(SF_SPRITE_ONCE) && !FClassnameIs(pev, "env_spritetrain"))
		{
			TurnOff();
		}
		else
...
void CSprite::TurnOn(void)
{
	...
	else if ((pev->framerate && m_maxFrame > 1.0) || HasSpawnFlags(SF_SPRITE_ONCE))
	{
		SetThink(&CSprite::AnimateThink);
		SetNextThink(0);
		m_lastTime = gpGlobals->time;
