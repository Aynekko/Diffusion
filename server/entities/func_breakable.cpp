...
// diffusion - I say we can spawn whatever we please :)
...
//		m_iShards = atof(pkvd->szValue);
	...
	else if (FStrEq(pkvd->szKeyName, "spawnobject") )
	{
		...
		if ( object > 0 && object < SIZEOFARRAY(pSpawnObjects) )
			m_iszSpawnObject = MAKE_STRING( pSpawnObjects[object] );
		else if( pkvd->szValue[0] != '0' )// diffusion - it can be a string
	...
	{
		RespawnTime = Q_atoi( pkvd->szValue );
		pkvd->fHandled = TRUE;
	}
	else if( FStrEq( pkvd->szKeyName, "healthbar" ) )
	{
		int healthbar = Q_atoi( pkvd->szValue );
		if( healthbar > 0 )
		{
			EnableHealthBar = true;
			HealthBarType = healthbar - 1;
		}
		pkvd->fHandled = TRUE;
	}
...
void CBreakable::Spawn( void )
{
	...
	pev->max_health = pev->health; // diffusion - remember health for respawn
...
const char **CBreakable::MaterialSoundList( Materials precacheMaterial, int &soundCount )
{
	...
	case matWood:
		...
		soundCount = SIZEOFARRAY(pSoundsWood);
		break;
	case matFlesh:
		pSoundList = pSoundsFlesh;
		soundCount = SIZEOFARRAY(pSoundsFlesh);
		break;
	case matComputer:
	case matUnbreakableGlass:
	case matGlass:
		pSoundList = pSoundsGlass;
		soundCount = SIZEOFARRAY(pSoundsGlass);
		break;

	case matMetal:
		pSoundList = pSoundsMetal;
		soundCount = SIZEOFARRAY(pSoundsMetal);
		break;

	case matCinderBlock:
	case matRocks:
		pSoundList = pSoundsConcrete;
		soundCount = SIZEOFARRAY(pSoundsConcrete);
		break;
...
void CBreakable::Use( CBaseEntity *pActivator, CBaseEntity *pCaller, USE_TYPE useType, float value )
{
	if ( IsBreakable() )
	{
		// diffusion - respawn the breakable by trigger
		if( RespawnTime == -1 && (pev->effects & EF_NODRAW))
		{
			BreakRespawn();
			return;
		}
...
void CBreakable::Die( CBaseEntity *pActivator )
{
	...
	// diffusion - remember it
	string_t Targetname = pev->targetname;
	pev->targetname = 0;
	...
	if( RespawnTime != 0 ) // -1 means respawn by Use
	{
		pev->effects |= EF_NODRAW;
		pev->targetname = Targetname;
		if( RespawnTime > 0 )
		{
			SetThink( &CBreakable::BreakRespawn );
...
//========================================================================
// diffusion - respawn the breakable after delay
//========================================================================
void CBreakable::BreakRespawn(void)
{
	// remove all the decals, attached to this entity
