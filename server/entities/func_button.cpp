...
BEGIN_DATADESC( CMultiSource )
	DEFINE_ARRAY( m_rgEntities, FIELD_EHANDLE, MAX_MASTER_TARGETS ),
	DEFINE_ARRAY( m_rgTriggered, FIELD_INTEGER, MAX_MASTER_TARGETS ),
	DEFINE_KEYFIELD( m_globalstate, FIELD_STRING, "globalstate" ),
...
{ 
	...
	// diffusion - support for brush trigger too
	pTarget = UTIL_FindEntityByClassname( NULL, "trigger_multimanager" );

	while( pTarget && (m_iTotal < MAX_MASTER_TARGETS) )
	{
		if( pTarget->HasTarget( pev->targetname ) )
			m_rgEntities[m_iTotal++] = pTarget;
		pTarget = UTIL_FindEntityByClassname( pTarget, "trigger_multimanager" );
	}
...
public:
	...
	// diffusion - this is undone and needs to be done properly. don't use this.
	// I made it only for the sake of rotating mirrors in the laser section.
	void ButtonContinue( void );
	void SimpleWait(void);
	
	virtual int TakeDamage( entvars_t* pevInflictor, entvars_t* pevAttacker, float flDamage, int bitsDamageType );
	
	enum BUTTON_CODE { BUTTON_NOTHING, BUTTON_ACTIVATE, BUTTON_RETURN, BUTTON_CONTINUE };
...
BEGIN_DATADESC( CBaseButton )
	...
	DEFINE_FUNCTION( ButtonContinue ),
...
int CBaseButton :: TakeDamage( entvars_t* pevInflictor, entvars_t* pevAttacker, float flDamage, int bitsDamageType )
{
	...
	if( pevAttacker ) // diffusion - fixed possible crash
...
//	if( m_flWait == 0 ) m_flWait = 1;
	...
	if( HasSpawnFlags(SF_BUTTON_SECRET))
...
void CBaseButton :: ButtonUse( CBaseEntity *pActivator, CBaseEntity *pCaller, USE_TYPE useType, float value )
{
	...
	if( HasSpawnFlags( SF_BUTTON_FREEZEPLAYER ) )
	{
		pActivator->SetAbsVelocity( g_vecZero );
		pActivator->SetFlag( F_PLAYER_BUTTONFREEZED );
	}
...
CBaseButton::BUTTON_CODE CBaseButton::ButtonResponseToTouch( void )
{
	...
	// diffusion - don't activate by shooting, wait
	if( m_iState != STATE_OFF && pev->takedamage == DAMAGE_YES )
		return BUTTON_NOTHING;
	...
	if( m_iState == STATE_ON )
	{
		...
		else if ( (pev->spawnflags & SF_BUTTON_TOGGLE) && (pev->spawnflags & SF_BUTTON_SAMEDIR) )
		{
//			m_iState = STATE_OFF;
...
void CBaseButton::TriggerAndWait( void )
{
	...
	{ 
		...
		else if ( FBitSet(pev->spawnflags, SF_BUTTON_SAMEDIR) )
		{
			SetTouch( NULL );
			if( !HasSpawnFlags(SF_BUTTON_SECRET ))
...
void CBaseButton::ButtonContinue( void )
{
	...
	if( m_flWait )
	{
		if( !HasSpawnFlags(SF_BUTTON_SECRET ))
			SetFlag(F_ENTITY_BUSY);
		SetThink( &CBaseButton::SimpleWait );
		SetNextThink( m_flWait );
	}
}

void CBaseButton::SimpleWait(void)
{
	if( !HasSpawnFlags(SF_BUTTON_SECRET ))
		RemoveFlag(F_ENTITY_BUSY);
	SetThink( NULL );
