...
#include "player.h"
#include "doors.h"
#include "entities/trains.h"		// for func_traindoor
...
BEGIN_DATADESC( CBaseDoor )
	...
	DEFINE_KEYFIELD( AutoDoorRadius, FIELD_INTEGER, "openradius" ),
	DEFINE_FIELD( AutoTime, FIELD_FLOAT ),  //	DEFINE_KEYFIELD( AutoTime, FIELD_FLOAT, "autotime" ),
	DEFINE_KEYFIELD( CustomOpenSpeed, FIELD_FLOAT, "speedopen" ),
	DEFINE_KEYFIELD( CustomCloseSpeed, FIELD_FLOAT, "speedclose" ),
	DEFINE_FIELD( FirstMovement, FIELD_INTEGER ),
	DEFINE_KEYFIELD( MoveDir2, FIELD_VECTOR, "movedir2" ),
	DEFINE_FIELD( m_vecPosition3, FIELD_VECTOR ),
	DEFINE_FUNCTION( SecondaryLinearMove ),
	DEFINE_KEYFIELD( SecondaryMoveSpeedScale, FIELD_FLOAT, "2ndspeedscale" ),
	DEFINE_KEYFIELD( m_flLip2, FIELD_FLOAT, "lip2" ),
	DEFINE_KEYFIELD( m_flAttenuation, FIELD_FLOAT, "soundradius" ),
END_DATADESC()
...
void CBaseDoor::KeyValue( KeyValueData *pkvd )
{
	...
	else if( FStrEq( pkvd->szKeyName, "openradius" )) // diffusion
	{
...
//		pkvd->fHandled = TRUE;
//	}
	else if( FStrEq( pkvd->szKeyName, "speedopen" ))
	{
		CustomOpenSpeed = Q_atof( pkvd->szValue );
		pkvd->fHandled = TRUE;
	}
	else if( FStrEq( pkvd->szKeyName, "speedclose" ))
	{
		CustomCloseSpeed = Q_atof( pkvd->szValue );
		pkvd->fHandled = TRUE;
	}
	else if( FStrEq( pkvd->szKeyName, "movedir2" ))
	{
		UTIL_StringToVector( MoveDir2, pkvd->szValue );
		pkvd->fHandled = TRUE;
	}
	else if( FStrEq( pkvd->szKeyName, "2ndspeedscale" ))
	{
		SecondaryMoveSpeedScale = Q_atof( pkvd->szValue );
		pkvd->fHandled = TRUE;
	}
	else if( FStrEq( pkvd->szKeyName, "lip2" ))
	{
		m_flLip2 = Q_atof( pkvd->szValue );
		pkvd->fHandled = TRUE;
	}
...
void CBaseDoor::Precache( void )
{
	...
	case 0: pev->noise1 = MAKE_STRING( "common/null.wav" ); break;
	default: pev->noise1 = UTIL_PrecacheSound( m_sound ); break;
	}
	...
	case 0: pev->noise2 = MAKE_STRING( "common/null.wav" ); break;
	default: pev->noise2 = UTIL_PrecacheSound( m_sound ); break;
	}
...
void CBaseDoor::Spawn( void )
{
	...
	*/
	...
	if( m_flWait == 0.0f || HasSpawnFlags(SF_DOOR_NO_AUTO_RETURN) ) // diffusion - also support for toggle doors. They don't need m_flWait
		m_flWait = -1;
	...
	// even if we don't have secondary move set, the door's speed is still calculated as pev->speed * scale
	if( !SecondaryMoveSpeedScale )
		SecondaryMoveSpeedScale = 1.0;

	// diffusion - the door will make one more open movement in different direction
	if( HasSpawnFlags( SF_DOOR_2NDMOVEMENT ) )
	{
		if( !MoveDir2 )
			MoveDir2 = Vector( -1.0f, 0.0f, 0.0f );

		FirstMovement = 0;
		UTIL_MakeVectors( MoveDir2 );
		MoveDir2 = gpGlobals->v_forward;
		m_vecPosition3 = m_vecPosition2 + (MoveDir2 * (DotProductAbs( MoveDir2, vecSize ) - m_flLip2));
//		ALERT(at_console, "%.2f\n", m_flLip2 );
	}

//	ALERT(at_console, "Pos1 %.2f %.2f %.2f\n", m_vecPosition1.x, m_vecPosition1.y, m_vecPosition1.z);
//	ALERT(at_console, "Pos2 %.2f %.2f %.2f\n", m_vecPosition2.x, m_vecPosition2.y, m_vecPosition2.z);
//	ALERT(at_console, "Pos3 %.2f %.2f %.2f\n", m_vecPosition3.x, m_vecPosition3.y, m_vecPosition3.z);

	if( HasSpawnFlags( SF_DOOR_START_OPEN ))
	{	
		if( HasSpawnFlags( SF_DOOR_2NDMOVEMENT ))
		{
			UTIL_SetOrigin( this, m_vecPosition3 );
			m_vecPosition3 = m_vecPosition1;
			m_vecPosition1 = GetLocalOrigin();
		}
		else
		{
	...
	// another hack: PhysX 2.8.4.0 crashed while trying to create kinematic body from this brush-model
	...
	if ( HasSpawnFlags( SF_DOOR_USE_ONLY ) && !HasSpawnFlags( SF_DOOR_FORCETOUCHABLE ))
		SetTouch ( NULL );
	else if ( HasSpawnFlags( SF_DOOR_AUTOMATED)) // diffusion
		SetTouch( NULL );
	else
	// touchable button
		SetTouch( &CBaseDoor::DoorTouch );

	if ( HasSpawnFlags(SF_DOOR_AUTOMATED) )
	{
		// we will need this as we activate the door with on-off use
		pev->spawnflags |= SF_DOOR_ONOFF_MODE;

		if (!AutoDoorRadius)
			AutoDoorRadius = 192;

		AutoTime = 0.2; // fine number I think

		SetThink( &CBaseDoor::DoorOpenThink );
		SetNextThink( RANDOM_FLOAT(0.5, 1.5) ); // spread think times
	}

	// need to get rid of this probably, it was just for test
	if( HasSpawnFlags( SF_DOOR_SPECIALEFFECTS ))
	{
		// start invisible
		pev->rendermode = kRenderFxHologram;
		pev->renderamt = 0;
	}
	
	if( !m_flAttenuation )
		m_flAttenuation = ATTN_NORM; // 0.8
}

//===============================================================================================
// DoorOpenThink: open the door if someone's in the radius
//===============================================================================================
void CBaseDoor :: DoorOpenThink ( void )
{
	/*
	UNDONE: need some kind of trace to prevent activation through walls.
	Tracing pList[0] was not a good idea (if player is near, he gets all the priority)
	and sometimes trace fails because secondary double door is in the way.
	*/
...
//			ALERT(at_console, "Locked by master!\n");
			SetNextThink(1); // door is locked anyway, do not think too often
		}
	}
	else
	{
//		ALERT(at_console, "no one is here yet...\n");
		if (!IsLockedByMaster( ))
		{
			SetThink (&CBaseDoor::DoorOpenThink );
			SetNextThink( AutoTime );
		}
		else
		{
			// no one is here and the door is locked. don't think too often
			SetThink (&CBaseDoor::DoorOpenThink );
			SetNextThink(2);
		}
	}
...
//===============================================================================================
// DoorCloseThink: close the door if nobody's in the radius
//===============================================================================================
...
//		ALERT(at_console, "someone is here\n");
		SetThink( &CBaseDoor::DoorCloseThink );
		SetNextThink( AutoTime );
	}
}
...
void CBaseDoor :: Activate( void )
{
	...
	int doorCount = GetDoorMovementGroup( pDoorList, SIZEOFARRAY( pDoorList ));
...
void CBaseDoor::DoorTouch( CBaseEntity *pOther )
{
	...
	if( !FStringNull( pev->targetname ) && FStringNull( m_iChainTarget ) && !HasSpawnFlags( SF_DOOR_FORCETOUCHABLE))
	{
		// play locked sound
		PlayLockSounds( pev, &m_ls, TRUE, FALSE );
		return; 
...
void CBaseDoor::Use( CBaseEntity *pActivator, CBaseEntity *pCaller, USE_TYPE useType, float value )
{
	...
	if( IsLockedByMaster( ) )
	{
		if( HasSpawnFlags( SF_DOOR_AUTOMATED ) && ( value == 777 ) )
		{
			// bypass master, we need to close the door
		}
		else
		{
			PlayLockSounds( pev, &m_ls, TRUE, FALSE );
	...
	if( HasSpawnFlags( SF_DOOR_ONOFF_MODE ))
	{
		if( useType == USE_ON )
		{
	...
	// diffusion - rewrote function for this flag a little bit
	...
	if( m_iState == STATE_OFF || m_iState == STATE_ON )
		DoorActivate();
	// if not ready to be used, ignore "use" command.
//	if( m_iState == STATE_OFF || HasSpawnFlags( SF_DOOR_NO_AUTO_RETURN ) && m_iState == STATE_ON )
//		DoorActivate();
}
...
BOOL CBaseDoor::DoorActivate( void )
{
	...
	if( HasSpawnFlags( SF_DOOR_NO_AUTO_RETURN ) && m_iState == STATE_ON )
	{
		// door should close
		DoorGoDown();
	}
...
void CBaseDoor::DoorGoUp( void )
{
	...
	// BUGBUG looping move sounds don't save!!!
	if( !HasSpawnFlags( SF_DOOR_SILENT ) )
		EMIT_SOUND( edict(), CHAN_STATIC, STRING( pev->noise1 ), 1, m_flAttenuation );
	...
	{
		FirstMovement = 1;
		SetMoveDone(&CBaseDoor::SecondaryLinearMove );
	}
	else
	...
	if( IsRotatingDoor( ))
	{
		...
		if( m_hActivator != NULL && m_bDoorTouched )
		{
			...
			if( !HasSpawnFlags( SF_DOOR_ONEWAY ) && pev->movedir.y )
			{
		...
		if( CustomOpenSpeed )
			AngularMove( m_vecAngle2 * sign, CustomOpenSpeed );
		else
	...
	else
	{
		...
		{
			if( HasSpawnFlags( SF_DOOR_START_OPEN) )
				LinearMove( m_vecPosition2, CustomOpenSpeed * SecondaryMoveSpeedScale );
			else
				LinearMove( m_vecPosition2, CustomOpenSpeed );
		}
		else
		{
			if( HasSpawnFlags( SF_DOOR_START_OPEN) )
				LinearMove( m_vecPosition2, pev->speed * SecondaryMoveSpeedScale );
			else
...
void CBaseDoor::SecondaryLinearMove(void)
{
	if( FirstMovement == 1 ) // door is in opening state
	{
		SetMoveDone(&CBaseDoor::DoorHitTop );
		if( CustomOpenSpeed )
		{
			if( HasSpawnFlags( SF_DOOR_START_OPEN) )
				LinearMove( m_vecPosition3, CustomOpenSpeed );
			else
				LinearMove( m_vecPosition3, CustomOpenSpeed * SecondaryMoveSpeedScale );
		}
		else
		{
			if( HasSpawnFlags( SF_DOOR_START_OPEN) )
				...
				LinearMove( m_vecPosition3, pev->speed * SecondaryMoveSpeedScale );
		}
	}
	else if( FirstMovement == -1 )
	{
		SetMoveDone(&CBaseDoor::DoorHitBottom ); // closing state
		if( CustomOpenSpeed )
		{
			if( HasSpawnFlags( SF_DOOR_START_OPEN) )
				LinearMove( m_vecPosition1, CustomOpenSpeed * SecondaryMoveSpeedScale );
			else
				LinearMove( m_vecPosition1, CustomOpenSpeed );
		}
		else
		{
			if( HasSpawnFlags( SF_DOOR_START_OPEN) )
				LinearMove( m_vecPosition1, pev->speed * SecondaryMoveSpeedScale );
			else
...
void CBaseDoor::DoorHitTop( void )
{
	if( !HasSpawnFlags( SF_DOOR_SILENT ))
	{
		...
		EMIT_SOUND( edict(), CHAN_STATIC, STRING( pev->noise2 ), 1, m_flAttenuation );
	}
	...
	{
		// in m_flWait seconds, DoorGoDown will fire, unless wait is -1, then door stays open
		...
		if( HasSpawnFlags( SF_DOOR_AUTOMATED ) )
		{
			...
			if (m_flWait < 0)
				m_flWait = 0;
			pev->nextthink = gpGlobals->time + AutoTime + m_flWait; // if the door is slow, it won't go down instantly
		}
		else
		{
...
void CBaseDoor::DoorGoDown( void )
{
	...
	{
		if( CustomCloseSpeed )
			AngularMove( m_vecAngle1, CustomCloseSpeed );
		else
	...
	else 
	{
		if( HasSpawnFlags( SF_DOOR_2NDMOVEMENT) )
		{
			...
			{
				if( HasSpawnFlags( SF_DOOR_START_OPEN ))
					LinearMove( m_vecPosition2, CustomCloseSpeed );
				else
					LinearMove( m_vecPosition2, CustomCloseSpeed * SecondaryMoveSpeedScale );
			}
			else
			{
				if( HasSpawnFlags( SF_DOOR_START_OPEN) )
		...
		{
			if( CustomCloseSpeed )
				LinearMove( m_vecPosition1, CustomCloseSpeed );
			else
				LinearMove( m_vecPosition1, pev->speed );
		}
	}
...
void CBaseDoor::DoorHitBottom( void )
{
	...
	if ( (HasSpawnFlags( SF_DOOR_USE_ONLY ) && !HasSpawnFlags( SF_DOOR_FORCETOUCHABLE )) || HasSpawnFlags( SF_DOOR_AUTOMATED) )
		// use only door
		SetTouch( NULL );
	else
		// touchable door
...
void CMomentaryDoor :: Use( CBaseEntity *pActivator, CBaseEntity *pCaller, USE_TYPE useType, float value )
{
	...
	// NOTE: historically momentary_door is completely ignoring pev->speed
	...
	if( HasSpawnFlags( SF_DOOR_CONSTANT_SPEED ))
		speed = pev->speed;
	else
	{	
...
void CBaseTrainDoor :: Use( CBaseEntity *pActivator, CBaseEntity *pCaller, USE_TYPE useType, float value )
{
	...
	if( HasSpawnFlags( SF_TRAINDOOR_ONOFF_MODE ))
          {
		if( useType == USE_ON )
		{
...
void CBaseTrainDoor::DoorGoUp( void )
{
	...
	if( HasSpawnFlags( SF_TRAINDOOR_INVERSE) )
		depth = -depth;
