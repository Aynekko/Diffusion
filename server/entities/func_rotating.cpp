...
void CFuncRotating::Spawn(void)
{
	...
	if ( HasSpawnFlags(SF_ROTATING_SND_SMALL_RADIUS))
	{
		m_flAttenuation = ATTN_IDLE;
	}
	else if ( HasSpawnFlags(SF_ROTATING_SND_MEDIUM_RADIUS))
	{
		m_flAttenuation = ATTN_STATIC;
	}
	else if ( HasSpawnFlags(SF_ROTATING_SND_LARGE_RADIUS))
	{
		m_flAttenuation = ATTN_NORM;
	}
	...
	if (HasSpawnFlags(SF_ROTATING_Z_AXIS))
		pev->movedir = Vector(0, 0, 1);
	else if ( HasSpawnFlags(SF_ROTATING_X_AXIS))
		pev->movedir = Vector(1, 0, 0);
	else
		pev->movedir = Vector(0, 1, 0); // y-axis

	// check for reverse rotation
	if ( HasSpawnFlags(SF_ROTATING_BACKWARDS))
	{
	...
	// instant-use brush?
	if ( HasSpawnFlags(SF_ROTATING_INSTANT))
	{
	...
	if ( HasSpawnFlags(SF_ROTATING_HURT))
	{
...
void CFuncRotating::Precache(void)
{
	...
	else
	{
		...
		case 5:
			...
			break;
		case 0:
			pev->noise3 = 0;// UTIL_PrecacheSound( "common/null.wav" ); // diffusion
			break;
		default:
...
void CFuncRotating::ClearEffects(void)
{
	DontThink();
	SetMoveDone( NULL );
	SetMoveDoneTime( -1 );
