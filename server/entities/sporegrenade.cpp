...
BEGIN_DATADESC( CSporeGrenade )
	DEFINE_FIELD(m_pSporeGlow, FIELD_CLASSPTR),
...
void CSporeGrenade::Explode(TraceResult *pTrace, int bitsDamageType)
{
	...
	CSoundEnt::InsertSound(bits_SOUND_COMBAT, pev->origin, NORMAL_EXPLOSION_VOLUME, 3.0, sndentindex);
	entvars_t *pevOwner;
	if (pev->owner)
	...
	RadiusDamage(pev, pevOwner, pev->dmg, pev->dmg * 2.5, CLASS_NONE, bitsDamageType); //DMG_BLAST

	// Place a decal on the surface that was hit.
	UTIL_DecalTrace(pTrace, DECAL_SPIT1 + RANDOM_LONG(0, 2)); // DECAL_SPR_SPLT1

	if (m_pSporeGlow)   // diffusion
	{
...
void CSporeGrenade::ExplodeTouch(CBaseEntity *pOther)
{
	...
	vecSpot = GetAbsOrigin() - GetAbsVelocity().Normalize() * 32;//vecSpot = pev->origin - pev->velocity.Normalize() * 32;
	UTIL_TraceLine( vecSpot, vecSpot + GetAbsVelocity().Normalize() * 64, ignore_monsters, edict(), &tr );//UTIL_TraceLine(vecSpot, vecSpot + pev->velocity.Normalize() * 64, ignore_monsters, ENT(pev), &tr);
...
void CSporeGrenade::Spawn(void)
{
	...
	pev->dmg = 75; //gSkillData.plrDmgSpore; ??? not working
...
CBaseEntity* CSporeGrenade::ShootTimed(entvars_t *pevOwner, Vector vecStart, Vector vecVelocity, bool ai)
{
	...
	pGrenade->Spawn();
	pGrenade->SetAbsVelocity( vecVelocity );//pGrenade->pev->velocity = vecVelocity;
	pGrenade->SetLocalAngles( UTIL_VecToAngles( pGrenade->GetAbsVelocity( )));//pGrenade->pev->angles = UTIL_VecToAngles(pGrenade->pev->velocity);
	...
	pGrenade->SetNextThink( 0.1 );//pGrenade->pev->nextthink = gpGlobals->time + 0.1;
	if (lifetime < 0.1)
	{
...
CBaseEntity *CSporeGrenade::ShootContact(entvars_t *pevOwner, Vector vecStart, Vector vecVelocity)
{
	...
	pGrenade->SetLocalVelocity( vecVelocity );//pGrenade->pev->velocity = vecVelocity;
	pGrenade->SetLocalAngles( UTIL_VecToAngles( pGrenade->GetLocalVelocity( )));//pGrenade->pev->angles = UTIL_VecToAngles(pGrenade->pev->velocity);
	pGrenade->pev->owner = ENT(pevOwner);

	// make monsters afraid of it while in the air
	pGrenade->SetThink(&CSporeGrenade::DangerSoundThink);
	pGrenade->SetNextThink( 0 );//pGrenade->pev->nextthink = gpGlobals->time;
