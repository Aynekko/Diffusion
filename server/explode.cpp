...
void CShower::Spawn( void )
{
	...
	pev->solid = SOLID_NOT;
	SetNullModel(); // diffusion - model needed
...
{ 
	...
	MESSAGE_BEGIN( MSG_PAS, gmsgTempEnt, absOrigin );
		...
		if( !HasSpawnFlags( SF_ENVEXPLOSION_NOSOUND ) )
			...
			WRITE_BYTE( TE_EXPLFLAG_NOSOUND ); // don't allow sound
	MESSAGE_END();

	// do damage
	if ( !HasSpawnFlags(SF_ENVEXPLOSION_NODAMAGE ) )
	{
		RadiusDamage ( pev->owner ? VARS( pev->owner ) : pev, pActivator ? pActivator->pev : pev, m_iMagnitude, m_iMagnitude * 2.5, CLASS_NONE, DMG_BLAST );
