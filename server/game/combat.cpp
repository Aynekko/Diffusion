...
void CBaseMonster :: GibMonster( void )
{
	...
	if (m_bloodColor != DONT_BLEED)
	{
		...
		{
			case 0: EMIT_SOUND(ENT(pev), CHAN_WEAPON, "common/bodysplat1.wav", 1, ATTN_NORM); break;
...
Activity CBaseMonster :: GetDeathActivity ( void )
{
	...
	if ( LookupActivity ( deathActivity ) == ACTIVITY_NOT_AVAILABLE )
	{
		...
		if ( fTriedDirection )
			deathActivity = ACT_DIESIMPLE; // if yes, we're out of options. Go simple.
		else
		{
	...
	if ( LookupActivity ( deathActivity ) == ACTIVITY_NOT_AVAILABLE )
		deathActivity = ACT_DIESIMPLE; // if we're still invalid, simple is our only option.
...
int CBaseMonster::DamageDecal( int bitsDamageType )
{
	...
	if( HasFlag(F_METAL_MONSTER) )
		return DECAL_GUNSHOT1 + RANDOM_LONG( 0, DECAL_GUNSHOT4 );
...
void CBaseMonster :: Killed( entvars_t *pevAttacker, int iGib )
{
	...
	EMIT_SOUND(ENT(pev), CHAN_WEAPON, "common/null.wav", 0, ATTN_NORM);
...
void CGib :: WaitTillLand ( void )
{
	...
	if( GetAbsVelocity() == g_vecZero )
	{
		...
		SetNextThink( 0.5 ); // wait and check again in another half second.
}
...
int CBaseMonster::TakeDamage( entvars_t *pevInflictor, entvars_t *pevAttacker, float flDamage, int bitsDamageType )
{
	...
	if( pevAttacker->flags & (FL_CLIENT|FL_MONSTER) )
		CSoundEnt::InsertSound ( bits_SOUND_COMBAT, GetAbsOrigin(), 512, 0.5 );

	// HACKHACK Don't kill monsters in a script.  Let them break their scripts first
	// diffusion - I need this disabled...
...
//	}
	...
	if ( pev->health <= 0 )
	{
		...
		if( pevAttacker->flags & FL_CLIENT )
		{
			...
			// multiplayer achievements - attacker is a client, and I'm a (fake-)client too
...
void RadiusDamage( Vector vecSrc, entvars_t *pevInflictor, entvars_t *pevAttacker, float flDamage, float flRadius, int iClassIgnore, int bitsDamageType )
{
	...
	if( flDamage == 0.0f ) // diffusion
		return;

	if ( flRadius )
	{
	//	flRadius = flRadius * 1.25; // diffusion
		falloff = flDamage / (flRadius * 1.25);
	}
	else
		falloff = 1.0;

//	ALERT(at_console, "flDamage %.2f, flRadius %.2f falloff %.2f\n", flDamage, flRadius, falloff);
	...
	{		
		if ( pEntity->pev->takedamage != DAMAGE_NO )
		{
			...
			if ( tr.flFraction == 1.0 || tr.pHit == pEntity->edict() )
			{
				...
				//	// diffusion - FIXME fixed blood incorrect spawn?
				//	// if the grenade or energy ball stuck in the monster and player got damage, player's blood would spawn in the monster instead. Very visible on big robo.
				//	tr.vecEndPos = pEntity->GetAbsOrigin();// vecSrc;
					tr.vecEndPos = vecSrc;
					tr.flFraction = 0.0;
				}
				...
				// diffusion - victim is in the radius, so always hit with something
				if ( flAdjustedDamage < flDamage / 10 )
					flAdjustedDamage = flDamage / 10;

				// diffusion - FIXME - don't adjust damage for breakables
				if( UTIL_GetModelType( pEntity->pev->modelindex ) == mod_brush )
					flAdjustedDamage = flDamage;
			
			//	ALERT( at_console, "hit %s flAdjustedDamage %.2f, flDamage was %.2f\n", STRING( pEntity->pev->classname ), flAdjustedDamage, flDamage );
				if (tr.flFraction != 1.0)
				{
					ClearMultiDamage( );
...
// diffusion - here I have added the radius back in both
...
void CBaseMonster :: RadiusDamage( Vector vecSrc, entvars_t *pevInflictor, entvars_t *pevAttacker, float flDamage, float flRadius, int iClassIgnore, int bitsDamageType )
{
...
//=========================================================
// MakeWaterSplash: do watersplash
//=========================================================
void CBaseEntity::MakeWaterSplash( Vector vecSrc, Vector vecEnd, int Type )
{
	...
	if( len <= 1 )
	{
		MESSAGE_BEGIN( MSG_PVS, gmsgWaterSplash, vecTemp );
	...
	else
	{
		if( POINT_CONTENTS( vecTemp ) == CONTENTS_WATER )
			MakeWaterSplash( vecSrc, vecTemp, Type );
		else
			MakeWaterSplash( vecTemp, vecEnd, Type );
	}
}
...
void CBaseEntity::FireBullets(ULONG cShots, Vector vecSrc, Vector vecDirShooting, Vector vecSpread, float flDistance, int iBulletType, int iTracerFreq, float iDamage, entvars_t *pevAttacker, edict_t *pentIgnore )
{
	...
	for (ULONG iShot = 1; iShot <= cShots; iShot++)
	{
		...
		if (iTracerFreq != 0 && (tracerCount++ % iTracerFreq) == 0)
		{
			...
			if ( IsPlayer() ) // adjust tracer position for player
		...
		if (tr.flFraction != 1.0)
		{
			...
			switch(iBulletType) // damage is fixed. no skill.cfg
			{
				case BULLET_PLAYER_CROWBAR:		BulletDamage = 10; RangeModifier = 1.0; break;
				case BULLET_MONSTER_9MM:		BulletDamage = 7; RangeModifier = 0.8; break;
			...
			if( BulletDamage <= 0 )
				ALERT( at_warning, "FireBullets: Registered a bullet shot without any damage!\n" );

			// diffusion - check for correct texture (skip sky)
			bool ShouldDecal = true;
			...
			{
				pEntity->TraceAttack(pevAttacker, BulletDamage, vecDir, &tr, DMG_SLASH);
				if( ShouldDecal )
				{
					TEXTURETYPE_PlaySound( &tr, vecSrc, vecEnd, iBulletType );
			...
			{
				pEntity->TraceAttack(pevAttacker, BulletDamage, vecDir, &tr, DMG_BULLET);
				if( ShouldDecal )
				{
					TEXTURETYPE_PlaySound( &tr, vecSrc, vecEnd, iBulletType );
					DecalGunshot( &tr, iBulletType );
...
void CBaseMonster :: MakeDamageBloodDecal ( int cCount, float flNoise, TraceResult *ptr, const Vector &vecDir )
{
	...
	if ( !IsAlive() )
	{
		...
		if ( pev->max_health <= 0 ) // no blood decal for a monster that has already decalled its limit.
			return; 
		else
			pev->max_health--;
	}
