...
//	{ TASK_FACE_IDEAL,			(float)0	}, // checkthisfaceideal
};
...
Schedule_t	slAlertSmallFlinch[] =
{
	{ 
		tlAlertSmallFlinch,
		SIZEOFARRAY ( tlAlertSmallFlinch ),
		0,
		0,
		"Alert Small Flinch"
	},
...
// diffusion - same thing, but running
Task_t tlInvestigateSoundRUN[] =
{
	{ TASK_STOP_MOVING,				(float)0				},
	{ TASK_STORE_LASTPOSITION,		(float)0				},
	{ TASK_GET_PATH_TO_BESTSOUND,	(float)32				},
...
Schedule_t	slInvestigateSoundRUN[] =
{
	{ 
		tlInvestigateSoundRUN,
...
Schedule_t	slPrimaryMeleeAttack[] =
{
	{ 
		...
		SIZEOFARRAY ( tlPrimaryMeleeAttack1 ), 
		bits_COND_NEW_ENEMY			|
		bits_COND_ENEMY_DEAD		|
		bits_COND_LIGHT_DAMAGE		|
		bits_COND_HEAVY_DAMAGE		|
...
Schedule_t	slSecondaryMeleeAttack[] =
{
	{ 
		...
		SIZEOFARRAY ( tlSecondaryMeleeAttack1 ), 
		bits_COND_NEW_ENEMY			|
		bits_COND_ENEMY_DEAD		|
		bits_COND_LIGHT_DAMAGE		|
		bits_COND_HEAVY_DAMAGE		|
...
Schedule_t	slChaseEnemyFailed[] =
{
	{ 
		tlChaseEnemyFailed,
		SIZEOFARRAY ( tlChaseEnemyFailed ), 
		bits_COND_NEW_ENEMY			|
		bits_COND_CAN_RANGE_ATTACK1	|
		bits_COND_CAN_MELEE_ATTACK1	|
		bits_COND_CAN_RANGE_ATTACK2	|
...
Schedule_t slMoveToEnemyLKPRUN[] =
{
	{
		tlMoveToEnemyLKPRUN,
		SIZEOFARRAY( tlMoveToEnemyLKPRUN ),
		bits_COND_NEW_ENEMY |
		bits_COND_LIGHT_DAMAGE |
		bits_COND_HEAVY_DAMAGE |
		bits_COND_CAN_ATTACK |
...
{
	{
		...
		SIZEOFARRAY ( tlBarnacleVictimGrab ),
		0,
		0,
		"Barnacle Victim"
	}
...
{
	{
		...
		SIZEOFARRAY ( tlBarnacleVictimChomp ),
		0,
		0,
		"Barnacle Chomp"
	}
...
Schedule_t	slTakeCoverFromOrigin[] =
{
	{ 
		...
		SIZEOFARRAY ( tlTakeCoverFromOrigin ), 
		bits_COND_NEW_ENEMY,
		0,
		"TakeCoverFromOrigin"
	},
...
Schedule_t	slTakeCoverFromBestSound[] =
{
	{ 
		...
		SIZEOFARRAY ( tlTakeCoverFromBestSound ), 
		bits_COND_NEW_ENEMY,
		0,
...
{
	{ TASK_STOP_MOVING,				(float)0					},
	{ TASK_WAIT,					(float)0.1					}, // diffusion - was 0.2 // AIFIX
	{ TASK_FIND_COVER_FROM_ENEMY,	(float)0					},
	{ TASK_RUN_PATH,				(float)0					},
	{ TASK_WAIT_FOR_MOVEMENT,		(float)0					},
	{ TASK_REMEMBER,				(float)bits_MEMORY_INCOVER	},
...
Schedule_t	slTakeCoverFromEnemy[] =
{
	{ 
		...
		SIZEOFARRAY ( tlTakeCoverFromEnemy ), 
		bits_COND_NEW_ENEMY,
		0,
		"tlTakeCoverFromEnemy"
	},
...
{
	return ScheduleInList( pName, m_scheduleList, SIZEOFARRAY(m_scheduleList) );
}
...
//				ALERT( at_aiconsole, "Starting script %s for %s\n", STRING( m_pCine->m_iszPlay ), STRING(pev->classname) );
	...
	case SCHED_INVESTIGATE_SOUND:
		{
			if( HasSpawnFlags(SF_MONSTER_PRISONER) ) // diffusion - don't do it when in prison
				return slFail;
