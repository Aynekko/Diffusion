...
#if 0
	if ( FClassnameIs( pev, "monster_alien_assassin" ) )// && FStrEq(GetTargetname(), "bridger4") )
	{
...
{
	if ( m_pSchedule == NULL )	
		return FALSE; // schedule is empty, and therefore not valid.
	...
	case TASK_FIND_NEAR_NODE_COVER_FROM_ENEMY:
		{
			...
			else if ( FindRunAway( m_hEnemy->GetAbsOrigin(), 0, pTask->flData ) ) // diffusion - by Freeslave
			{
				TaskComplete();
			}
			else
	...
	case TASK_FIND_FAR_NODE_COVER_FROM_ENEMY:
		{
			...
			else if ( FindRunAway( m_hEnemy->GetAbsOrigin(), pTask->flData, CoverRadius() ) ) // diffusion - by Freeslave
			{
				TaskComplete();
			}
			else
	...
	case TASK_FIND_NODE_COVER_FROM_ENEMY:
		{
			...
			else if ( FindRunAway( m_hEnemy->GetAbsOrigin(), 0, CoverRadius() ) ) // diffusion - by Freeslave
			{
				TaskComplete();
			}
			else
...
//				return;
			...
			else if ( FindRunAway( pCover->GetAbsOrigin(), 0, CoverRadius() ) ) // diffusion - by Freeslave
			{
				TaskComplete();
			}
			else
	...
	case TASK_FIND_COVER_FROM_BEST_SOUND:
		{
			...
			*/
			...
			else if ( pBestSound && FindRunAway( pBestSound->m_vecOrigin, pBestSound->m_iVolume, CoverRadius() ) ) // diffusion - by Freeslave
			{
				m_flMoveWaitFinished = gpGlobals->time + pTask->flData;
				TaskComplete();
			}
	...
	case TASK_WALK_TO_TARGET:
		{
			...
			else
			{
				...
				else 
				{
					...
					{
						TaskFail();
						ALERT( at_aiconsole, "%s \"%s\" failed to reach target!!!\n", GetClassname(), GetTargetname() );
						RouteClear();
					}
				}
			}
	...
	case MONSTERSTATE_IDLE:
		{
			...
			else if( HasFlag(F_ENTITY_ONFIRE) )
	...
	case MONSTERSTATE_ALERT:
		{
			...
			{
				ALERT(at_console, "enemy lost, moving to LKP\n");
	...
	case MONSTERSTATE_COMBAT:
		{
			...
			else  
			{
				...
				if( HasConditions( bits_COND_SEETHROUGHWALL ) )
					return GetScheduleOfType( SCHED_CHASE_ENEMY );
				...
				if ( !HasConditions(bits_COND_CAN_RANGE_ATTACK1 | bits_COND_CAN_MELEE_ATTACK1) )
					return GetScheduleOfType( SCHED_CHASE_ENEMY ); // if we can see enemy but can't use either attack type, we must need to get closer to enemy
				else if ( !FacingIdeal() )
