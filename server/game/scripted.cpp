...
//		m_startTime = gpGlobals->time + 1E6;
	...
	if ( HasSpawnFlags(SF_SCRIPT_NOINTERRUPT) )
...
void CCineMonster :: PossessEntity( void )
{
	...
	// FindEntity() just checked this!
		...
		case 0: 
			...
			if( HasSpawnFlags( SF_SCRIPT_FORCELOOPIDLE ) )
				m_startTime = gpGlobals->time + 1E6; // diffusion - loop idle if no movement
			break;
...
BOOL CBaseMonster :: CineCleanup( void )
{
	...
	//	m_Activity = ACT_RESET; // diffusion - NextDay https://github.com/SNMetamorph/PrimeXT/issues/115#issuecomment-1370980987
	}
...
private:
	...
	// diffusion - subtitle, kind of... (env_message basically)
	string_t msg;
	string_t name;

	string_t SpeakFrom; // name of the entity, where the sound will be heard from
};
...
BEGIN_DATADESC( CScriptedSentence )
	...
	DEFINE_KEYFIELD( Attenuation, FIELD_FLOAT, "attenuation" ),
	...
	DEFINE_KEYFIELD( msg, FIELD_STRING, "msg" ),
	DEFINE_KEYFIELD( name, FIELD_STRING, "name" ),
	DEFINE_KEYFIELD( SpeakFrom, FIELD_STRING, "speakfrom" ),
END_DATADESC()
...
void CScriptedSentence :: KeyValue( KeyValueData *pkvd )
{
	...
	{
		Attenuation = Q_atof( pkvd->szValue );
		pkvd->fHandled = TRUE;
	}
	...
	else if (FStrEq(pkvd->szKeyName, "msg"))
	{
		msg = ALLOC_STRING(pkvd->szValue);
		pkvd->fHandled = TRUE;
	}
	else if (FStrEq(pkvd->szKeyName, "name"))
	{
		name = ALLOC_STRING(pkvd->szValue);
		pkvd->fHandled = TRUE;
	}
	else if( FStrEq( pkvd->szKeyName, "speakfrom" ) )
	{
		SpeakFrom = ALLOC_STRING( pkvd->szValue );
		pkvd->fHandled = TRUE;
	}
	else
...
BOOL CScriptedSentence :: StartSentence( CBaseMonster *pTarget )
{
	...
	if (!FStringNull(m_iszListener))
	{
		...
		if ( FStrEq( STRING(m_iszListener ), "player" ) )
			radius = 32768;	// Always find the player // diffusion - increased from 4096

		pListener = UTIL_FindEntityGeneric( STRING( m_iszListener ), pTarget->GetAbsOrigin(), radius );
	}

	pTarget->PlayScriptedSentence( STRING( m_iszSentence ), m_flDuration, m_flVolume, m_flAttenuation, bConcurrent, pListener, SpeakFrom );
