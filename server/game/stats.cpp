...
float AmmoDamage( const char *pName )
{
	...
	// diffusion BUGBUG - I'm not sure if the stats file is used at all, but anyway -
	// I got rid of skill.cfg for weapons, so this file must be adjusted somehow - I have distance-based damage right now
	if ( !strcmp( pName, "9mm" ) )
