...
public:
	...
	float		m_flDistLook;	// distance monster sees (Default 3072)
	float		m_flDistTooClose; // diffusion
	bool HealthbarDisabled;
...
// stuff written for new state machine
		...
		virtual BOOL FInViewCone ( CBaseEntity *pEntity, float m_flFieldOfView );// see if pEntity is in monster's view cone
		...
		virtual void PlaySentence( const char *pszSentence, float duration, float volume, float attenuation, string_t SpeakFrom );
		virtual void PlayScriptedSentence( const char *pszSentence, float duration, float volume, float attenuation, BOOL bConcurrent, CBaseEntity *pListener, string_t SpeakFrom );
