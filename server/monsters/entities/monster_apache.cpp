...
public:
	...
	float RotorVolume; // diffusion - for smoothing
};
...
void CApache :: Spawn( void )
{
	...
	if( !pev->health ) pev->health = g_ApacheHealth[g_iSkillLevel];
...
void CApache::Precache( void )
{
	...
	PRECACHE_SOUND("apache/ap_rotor.wav");
...
void CApache :: ClearEffects(void)
{
	STOP_SOUND( ENT(pev), CHAN_STATIC, "apache/ap_rotor.wav" );
}
...
void CApache :: HuntThink( void )
{
	...
	// diffusion - find an incoming rocket - it's in priority
	CBaseEntity *pIncomingRocket = NULL;
	while( (pIncomingRocket = UTIL_FindEntityInSphere( pIncomingRocket, GetAbsOrigin(), 1500 )) != NULL )
	{
		if( FClassnameIs( pIncomingRocket, "rpg_rocket" ) )
		{
			m_hEnemy = pIncomingRocket;
			break;
		}
	}
...
void CApache :: Flight( void )
{
	...
	{
		if( RotorVolume < 1.0f )
			RotorVolume += gpGlobals->frametime;
		...
		if (m_iSoundState == 0)
		{
			EMIT_SOUND_DYN(ENT(pev), CHAN_STATIC, "apache/ap_rotor.wav", RotorVolume, 0.2, 0, 110 );
		...
		else
		{
			...
			if (pPlayer)
			{
				...
				EMIT_SOUND_DYN(ENT(pev), CHAN_STATIC, "apache/ap_rotor.wav", RotorVolume, 0.2, SND_CHANGE_PITCH | SND_CHANGE_VOL, pitch);
			}
...
void CApache :: FireRocket( void )
{
	...
	// diffusion - fix https://github.com/ValveSoftware/halflife/issues/3208
	CBaseEntity *pRocket = CBaseEntity::Create( "hvr_rocket", vecSrc, GetAbsAngles(), edict() );

	if( pRocket )
	{
		MESSAGE_BEGIN( MSG_PVS, gmsgTempEnt, vecSrc );
		WRITE_BYTE( TE_SMOKE );
...
BOOL CApache :: FireGun( )
{
	...
	if (DotProduct( vecGun, vecTarget ) > 0.95) // diffusion - was 0.98
	{
#if 1
		if( m_hEnemy && FClassnameIs( m_hEnemy, "rpg_rocket"))
			FireBullets( 1, posGun, vecGun, VECTOR_CONE_1DEGREES, 8192, BULLET_MONSTER_12MM, 1, 5 );
		else
			FireBullets( 1, posGun, vecGun, VECTOR_CONE_4DEGREES, 8192, BULLET_MONSTER_12MM, 1, 5 );

		// MAKE 3 SOUNDS ON CLIENT !!!!!
		EMIT_SOUND_DYN( ENT( pev ), CHAN_WEAPON, "robo/shoot1.wav", 1, 0.2, 0, RANDOM_LONG( 90, 110 ) );
#else
		static float flNext;
		TraceResult tr;
...
int CApache :: TakeDamage( entvars_t* pevInflictor, entvars_t* pevAttacker, float flDamage, int bitsDamageType )
{
	...
	*/
	...
	// ALERT( at_console, "%.0f\n", flDamage );
	const int result = CBaseEntity::TakeDamage( pevInflictor, pevAttacker, flDamage, bitsDamageType );
