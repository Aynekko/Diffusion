...
BOOL CBullsquid :: FValidateHintType ( short sHint )
{
	...
	for ( i = 0 ; i < SIZEOFARRAY ( sSquidHints ) ; i++ )
	{
...
void CBullsquid :: HandleAnimEvent( MonsterEvent_t *pEvent )
{
	switch( pEvent->event )
	{
		case BSQUID_AE_SPIT:
		{
			...
			vecSpitDir = ( ( m_hEnemy->EyePosition() ) - vecSpitOffset ).Normalize();
...
Schedule_t	slSquidRangeAttack1[] =
{
	{ 
		tlSquidRangeAttack1,
		SIZEOFARRAY ( tlSquidRangeAttack1 ), 
		bits_COND_NEW_ENEMY			|
...
Schedule_t slSquidChaseEnemy[] =
{
	{ 
		tlSquidChaseEnemy1,
		SIZEOFARRAY ( tlSquidChaseEnemy1 ),
		bits_COND_NEW_ENEMY			|
		bits_COND_ENEMY_DEAD		|
...
Schedule_t slSquidSniffAndEat[] =
{
	{
		tlSquidSniffAndEat,
		SIZEOFARRAY( tlSquidSniffAndEat ),
		bits_COND_LIGHT_DAMAGE	|
		bits_COND_HEAVY_DAMAGE	|
		bits_COND_NEW_ENEMY	,
