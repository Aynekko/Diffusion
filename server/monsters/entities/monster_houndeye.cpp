...
BOOL CHoundeye :: FValidateHintType ( short sHint )
{
	...
	for ( i = 0 ; i < SIZEOFARRAY ( sHoundHints ) ; i++ )
	{
		...
		case HOUND_AE_HOPBACK:
			{
				...
				Vector vecVelocity = gpGlobals->v_forward * -200;
...
void CHoundeye :: SonicAttack ( void )
{
	...
	MESSAGE_BEGIN( MSG_PAS, SVC_TEMPENTITY, vecOrigin );
		...
		WRITE_COORD( vecOrigin.z + 16 );
		WRITE_COORD( vecOrigin.x );
		WRITE_COORD( vecOrigin.y );
		WRITE_COORD( vecOrigin.z + 16 + HOUNDEYE_MAX_ATTACK_RADIUS / .2 ); // reach damage radius over .3 seconds
	...
	MESSAGE_BEGIN( MSG_PAS, SVC_TEMPENTITY, vecOrigin );
		...
		WRITE_COORD( vecOrigin.x );
		WRITE_COORD( vecOrigin.y );
		WRITE_COORD( vecOrigin.z + 16 + ( HOUNDEYE_MAX_ATTACK_RADIUS / 2 ) / .2); // reach damage radius over .3 seconds
		WRITE_SHORT( m_iSpriteTexture );
		WRITE_BYTE( 0 ); // startframe
		WRITE_BYTE( 0 ); // framerate
		WRITE_BYTE( 2 ); // life
	...
	while ((pEntity = UTIL_FindEntityInSphere( pEntity, vecOrigin, HOUNDEYE_MAX_ATTACK_RADIUS )) != NULL)
	{
		if ( pEntity->pev->takedamage != DAMAGE_NO )
		{
			...
			{// houndeyes don't hurt other houndeyes with their attack
				...
				flDist = (pEntity->Center() - vecOrigin).Length();
...
Schedule_t	slHoundSpecialAttack1[] =
{
	{ 
		...
		SIZEOFARRAY ( tlHoundSpecialAttack1 ), 
		bits_COND_NEW_ENEMY			|
		bits_COND_LIGHT_DAMAGE		|
		bits_COND_HEAVY_DAMAGE		|
...
Schedule_t	slHoundCombatFailPVS[] =
{
	{ 
		...
		SIZEOFARRAY ( tlHoundCombatFailPVS ), 
		bits_COND_NEW_ENEMY			|
		bits_COND_LIGHT_DAMAGE		|
		bits_COND_HEAVY_DAMAGE,
		0,
...
Schedule_t	slHoundCombatFailNoPVS[] =
{
	{ 
		...
		SIZEOFARRAY ( tlHoundCombatFailNoPVS ), 
		bits_COND_NEW_ENEMY			|
		bits_COND_LIGHT_DAMAGE		|
		bits_COND_HEAVY_DAMAGE,
		0,
...
// dead enemy
			...
			if ( HasConditions( bits_COND_LIGHT_DAMAGE | bits_COND_HEAVY_DAMAGE ) )
			{
				if ( RANDOM_FLOAT( 0 , 1 ) <= 0.4 )
				{
					...
					UTIL_TraceHull( GetAbsOrigin(), GetAbsOrigin() + gpGlobals->v_forward * -128, dont_ignore_monsters, head_hull, ENT( pev ), &tr );
