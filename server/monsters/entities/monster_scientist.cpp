...
int CScientist::ObjectCaps(void)
{
...
Schedule_t	slFaceTargetScared[] =
{
	{
		tlFaceTargetScared,
		SIZEOFARRAY ( tlFaceTargetScared ),
		bits_COND_HEAR_SOUND |
		bits_COND_NEW_ENEMY,
		bits_SOUND_DANGER,
		"FaceTargetScared"
...
Schedule_t	slScientistStartle[] =
{
	{ 
		tlScientistStartle,
		SIZEOFARRAY ( tlScientistStartle ), 
		bits_COND_NEW_ENEMY |
		bits_COND_SEE_ENEMY |
		bits_COND_SEE_HATE |
		bits_COND_SEE_FEAR |
...
void CScientist :: Scream( void )
{
	if ( FOkToSpeak() )
	{
		...
		if( m_iszSpeakAs )
		{
			...
			strcat( szBuf, "_SCREAM" );
			PlaySentence( szBuf, RANDOM_FLOAT( 3, 6 ), VOL_NORM, ATTN_NORM );
		}
		else
		...
		if ( m_hEnemy && FOkToSpeak() )
		{
			...
			if( m_hEnemy->IsPlayer() )
			{
				if( m_iszSpeakAs )
				{
					char szBuf[32];
					strcpy( szBuf, STRING( m_iszSpeakAs ) );
					strcat( szBuf, "_PLFEAR" );
					PlaySentence( szBuf, 5, VOL_NORM, ATTN_NORM );
				}
				else
...
void CScientist :: Spawn( void )
{
	...
	if (pev->model)
		...
		SET_MODEL(ENT(pev), "models/npc/scientist.mdl");
...
void CDeadScientist :: Spawn( )
{
	...
	if( pev->sequence == -1 )
		ALERT( at_aiconsole, "Dead scientist with bad pose\n" );

	//	pev->skin += 2; // use bloody skin -- UNDONE: Turn this back on when we have a bloody skin again!
	MonsterInitDead();
}

int CDeadScientist :: TakeDamage( entvars_t *pevInflictor, entvars_t *pevAttacker, float flDamage, int bitsDamageType)
{
	if( HasSpawnFlags( SF_MONSTER_NODAMAGE ) )
		return 0;
...
void CSittingScientist :: Spawn( )
{
	...
	pev->solid			= SOLID_SLIDEBOX;
	pev->movetype		= MOVETYPE_STEP; // diffusion - just hang in the air
	pev->effects		= 0;
	pev->health			= 50;
...
//	UTIL_DropToFloor( this ); // diffusion - just hang in the air
}
...
// diffusion - female version
class CScientistFemale : public CScientist
{
	DECLARE_CLASS( CScientistFemale, CScientist );
public:
	void Spawn( void );
	void Precache( void );
};

LINK_ENTITY_TO_CLASS( monster_scientist_female, CScientistFemale );

//=========================================================
// Precache - precaches all resources this monster needs
//=========================================================
void CScientistFemale::Precache( void )
{
	if( pev->model )
		PRECACHE_MODEL( (char *)STRING( pev->model ) ); //LRC
	else
		PRECACHE_MODEL( "models/npc/scientist_f.mdl" );
	PRECACHE_SOUND( "scientist/sci_pain1.wav" );
	PRECACHE_SOUND( "scientist/sci_pain2.wav" );
	PRECACHE_SOUND( "scientist/sci_pain3.wav" );
	PRECACHE_SOUND( "scientist/sci_pain4.wav" );
	...
	if( !m_iszSpeakAs ) // diffusion - set female stuff
		m_iszSpeakAs = ALLOC_STRING( "SCF" );
...
void CScientistFemale::Spawn( void )
{
	Precache();
	...
	if( pev->model )
		...
		SET_MODEL( ENT( pev ), "models/npc/scientist_f.mdl" );
	UTIL_SetSize( pev, VEC_HUMAN_HULL_MIN, VEC_HUMAN_HULL_MAX );
...
class CSittingScientistFemale : public CSittingScientist
{
	DECLARE_CLASS( CSittingScientistFemale, CSittingScientist );
public:
	void Spawn( void );
	void Precache( void );
};

LINK_ENTITY_TO_CLASS( monster_sitting_scientist_f, CSittingScientistFemale );

//
// ********** Scientist SPAWN **********
//
void CSittingScientistFemale::Spawn()
{
	if( !m_iszSpeakAs ) // diffusion - set female stuff
		m_iszSpeakAs = ALLOC_STRING( "SCF" );
...
void CSittingScientistFemale::Precache( void )
{
	if( pev->model )
		PRECACHE_MODEL( (char *)STRING( pev->model ) ); //LRC
	else
...
class CDeadScientistFemale : public CDeadScientist
{
public:
	void Spawn( void );
};

LINK_ENTITY_TO_CLASS( monster_scientist_dead_f, CDeadScientistFemale );

//
// ********** DeadScientist SPAWN **********
//
void CDeadScientistFemale::Spawn()
{
	if( pev->model )
		PRECACHE_MODEL( (char *)STRING( pev->model ) ); //LRC
	else
