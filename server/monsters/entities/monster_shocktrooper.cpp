...
#define STROOPER_SHOCKRIFLE			1//(1 << 0)
#define STROOPER_HANDGRENADE		2//(1 << 1)
...
{
	DECLARE_CLASS(CStrooper, CHGrunt);
public:
	void Spawn(void);
	void MonsterThink();
	void Precache(void);
	int  Classify(void);
	void RunAI(void); // invisibility
...
BEGIN_DATADESC( CStrooper )
	DEFINE_FIELD(m_fRightClaw, FIELD_BOOLEAN),
	DEFINE_FIELD(m_rechargeTime, FIELD_TIME),
	DEFINE_FIELD(m_blinkTime, FIELD_TIME),
	DEFINE_FIELD(m_eyeChangeTime, FIELD_TIME),
END_DATADESC()
...
void CStrooper::GibMonster(void)
{
	...
	*/
	...
	{
		CGib::SpawnRandomGibs( pev, 6, 0 );	// Throw alien gibs           STROOPER_GIB_COUNT
	}
...
{
	if( !HasWeapon( STROOPER_HANDGRENADE ) )
	{
		return FALSE;
	}
...
void CStrooper :: RunAI( void ) // invisibility
{
	CBaseMonster :: RunAI();

		// always visible if moving
		// (always visible is not on hard) <- obsolete, for Diffusion invisibility is always available regardless of skill
...
void CStrooper::Spawn()
{
	...
	if (!m_bHaveWeapons)
	{
		...
		AddWeapon(STROOPER_SHOCKRIFLE);
		AddWeapon(STROOPER_HANDGRENADE);
	}
...
Schedule_t *CStrooper::GetSchedule(void)
{
	...
	if (HasConditions(bits_COND_HEAR_SOUND))
	{
		...
		if (pSound)
		{
			...
			*/
			if ( pSound && (pSound->m_iType & bits_SOUND_COMBAT) && (CanInvestigate == 1) )
