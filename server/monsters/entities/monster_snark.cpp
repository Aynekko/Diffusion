...
void CSqueakGrenade :: Killed( entvars_t *pevAttacker, int iGib )
{
	...
	if (m_hOwner != NULL)
		RadiusDamage ( pev, m_hOwner->pev, pev->dmg, pev->dmg * 2.5, CLASS_NONE, DMG_BLAST );
	else
		RadiusDamage ( pev, pev, pev->dmg, pev->dmg * 2.5, CLASS_NONE, DMG_BLAST );
...
{
	EMIT_SOUND_DYN(ENT(pev), CHAN_VOICE, "common/bodysplat1.wav", 0.75, ATTN_NORM, 0, 200);		
}
