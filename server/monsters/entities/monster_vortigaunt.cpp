...
BEGIN_DATADESC( CISlave )
	...
	DEFINE_AUTO_ARRAY( m_pBeam, FIELD_CLASSPTR ),
...
void CISlave :: PainSound( void )
{
	...
	{
		if (RANDOM_LONG( 0, 2 ) == 0)
			EMIT_SOUND_DYN ( ENT(pev), CHAN_WEAPON, pPainSounds[ RANDOM_LONG(0,SIZEOFARRAY(pPainSounds)-1) ], 1.0, ATTN_NORM, 0, m_voicePitch );
	}
}
...
{
	if( !(pev->spawnflags & SF_MONSTER_GAG) )
		EMIT_SOUND_DYN ( ENT(pev), CHAN_WEAPON, pDeathSounds[ RANDOM_LONG(0,SIZEOFARRAY(pDeathSounds)-1) ], 1.0, ATTN_NORM, 0, m_voicePitch );
}
	...
	switch( pEvent->event )
	{
		case ISLAVE_AE_CLAW:
		{
			...
			if ( pHurt )
			{
				...
				EMIT_SOUND_DYN ( ENT(pev), CHAN_WEAPON, pAttackHitSounds[ RANDOM_LONG(0,SIZEOFARRAY(pAttackHitSounds)-1) ], 1.0, ATTN_NORM, 0, m_voicePitch );
			}
			else
			{
				// Play a random attack miss sound
				EMIT_SOUND_DYN ( ENT(pev), CHAN_WEAPON, pAttackMissSounds[ RANDOM_LONG(0,SIZEOFARRAY(pAttackMissSounds)-1) ], 1.0, ATTN_NORM, 0, m_voicePitch );
			}
		}
		break;
		...
		//	pev->skin = m_iBeams / 2; // diffusion - remove
		}
		break;
...
void CISlave :: Spawn()
{
	...
	if (pev->model)
		...
		SET_MODEL(ENT(pev), "models/npc/dweller.mdl");
...
void CISlave :: Precache()
{
	...
	if (pev->model)
		...
		PRECACHE_MODEL("models/npc/dweller.mdl");
	...
	for ( i = 0; i < SIZEOFARRAY( pDeathSounds ); i++ )
...
//=========================================================
// ClearEffects - remove all beams
//=========================================================
void CISlave :: ClearEffects( )
{
