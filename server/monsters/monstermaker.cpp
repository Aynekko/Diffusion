...
#define	SF_MONSTERMAKER_START_ON	BIT(0) // start active ( if has targetname )
#define	SF_MONSTERMAKER_CYCLIC		BIT(2) // drop one monster every time fired.
#define SF_MONSTERMAKER_MONSTERCLIP	BIT(3) // Children are blocked by monsterclip
#define SF_MONSTERMAKER_DONTFORGET	BIT(4) // diffusion - set flag "can't lose enemy" for monster
...
void CMonsterMaker::MakeMonster( void )
{
	...
	if( HasSpawnFlags( SF_MONSTERMAKER_DONTFORGET ) ) // diffusion
		SetBits( pevCreate->spawnflags, SF_MONSTER_DONTFORGET );
	...
	// diffusion - fire target after you actually made a monster.
