...
BEGIN_DATADESC( CBaseMonster )
	...
	DEFINE_ARRAY( m_iWeapons, FIELD_CHARACTER, MAX_WEAPON_BYTES ),
	...
	DEFINE_KEYFIELD( m_flDistTooFar, FIELD_FLOAT, "m_flDistTooFar" ), // diffusion - now keyfields to override
	DEFINE_KEYFIELD( m_flDistLook, FIELD_FLOAT, "m_flDistLook" ),
	DEFINE_KEYFIELD( HealthbarDisabled, FIELD_BOOLEAN, "nohealthbar" ),
...
void CBaseMonster :: Listen ( void )
{
	...
	// diffusion - don't care about sounds if prisoner
	...
	while ( iSound != SOUNDLIST_EMPTY )
	{
		...
		if( pCurrentSound
			...
			&& (pCurrentSound->m_entindex != ENTINDEX( edict() ) ) // diffusion - skip "sounds" with my own index (I emitted it so I don't care about it)
		)
...
{
	if( ai_draw_route.value == 0 ) // diffusion - added cvar
		return;
...
//=====================================================================
// RouteSimplify: attempts to make the route more direct by cutting out
...
int CBaseMonster :: CheckEnemy ( CBaseEntity *pEnemy )
{
	...
	/* diffusion BUGBUG FIXME: I added occlusion time so the grunts would suppress the player
	if he goes in and out of cover repeatedly. Before this, the monster would simply look at the player,
	which allowed the player to go in and out of cover while shooting the monster - a possibility to shoot and
	never getting any damage at all.
	But here's a bug: sometimes, for an unknown reason, the monster gets into a loop (in particular, grunts)
	where he doesn't see the player and tries to choose Combat Face all the time, or Establish Line of Fire.
	...
	*/
	...
	//	ASSERT(!HasConditions(bits_COND_SEE_ENEMY));
		if (( gpGlobals->time >= m_flOcclusionTime + 1.5f) && !(HasConditions(bits_COND_ENEMY_OCCLUDED)) )
		{
	...
	else
	{
		...
		m_flOcclusionTime = gpGlobals->time - 0.5;
	}
...
//	ALERT( at_console, "%f %f\n", m_flOcclusionTime, gpGlobals->time );
	...
	if ( HasConditions( bits_COND_SEE_ENEMY ) )
	{
		...
		if ( pEnemyMonster )
		{
			if ( pEnemyMonster->FInViewCone ( this, m_flFieldOfView ) )
	...
	{
		if( !HasFlag( F_MONSTER_CANT_LOSE_ENEMY ) )
		{
			if( m_flLastTimeObservedEnemy + 45 <= gpGlobals->time )
			{
				SetConditions( bits_COND_ENEMY_LOST );
				ClearConditions( bits_COND_ENEMY_OCCLUDED );
				return FALSE;
	...
	// diffusion
	if ( flDistToEnemy <= m_flDistTooClose )
	{
		// enemy is too close
		SetConditions( bits_COND_ENEMY_TOOCLOSE );
	}
	else
		ClearConditions( bits_COND_ENEMY_TOOCLOSE );
...
void CBaseMonster::SetEnemy( CBaseEntity *pNewEnemy )
{
	...
	m_hEnemy = pNewEnemy;
	m_vecEnemyLKP = pNewEnemy->GetAbsOrigin();
	m_flLastTimeObservedEnemy = gpGlobals->time;
	...
	else
	{
		if ( ! (m_Route[ m_iRouteIndex ].iType & bits_MF_IS_GOAL) )
		{
			   ...
			   && (m_Route[m_iRouteIndex+1].iType & bits_MF_TO_NODE) == bits_MF_TO_NODE)
			{
				...
				{
				//	ALERT(at_console, "A link.\n");
					if ( WorldGraph.HandleLinkEnt ( iSrcNode, WorldGraph.m_pLinkPool[iLink].m_pLinkEnt, m_afCapability, CGraph::NODEGRAPH_DYNAMIC ) )
					{
					//	ALERT(at_console, "usable.\n");
						entvars_t *pevEnt = WorldGraph.m_pLinkPool[iLink].m_pLinkEnt;		
						if (pevEnt)
						{
							if( FClassnameIs(pevEnt, "func_door") || FClassnameIs(pevEnt, "func_door_rotating") )
							{
								if( !(pevEnt->spawnflags & SF_DOOR_NO_AUTO_RETURN) && !(pevEnt->spawnflags & SF_DOOR_AUTOMATED) )
								{
									m_flMoveWaitFinished = OpenDoorAndWait( pevEnt );
									ALERT( at_aiconsole, "Waiting for door %.2f\n", m_flMoveWaitFinished-gpGlobals->time );
								}
							}
						}
					}
					else if ( WorldGraph.HandleLinkEnt ( iSrcNode, WorldGraph.m_pLinkPool[iLink].m_pLinkEnt, m_afCapability, CGraph::NODEGRAPH_STATIC ) )
					{
						...
						// diffusion - handle a breakable on the way
...
BOOL CBaseMonster :: BuildRoute ( const Vector &vecGoal, int iMoveFlag, CBaseEntity *pTarget )
{
	...
	if ( iLocalMove == LOCALMOVE_VALID )
		return TRUE; // monster can walk straight there!
	...
	{
		//ALERT ( at_console, "BuildRoute: Can get there on nodes\n" );
...
BOOL CBaseMonster :: FTriangulate ( const Vector &vecStart , const Vector &vecEnd, float flDist, CBaseEntity *pTargetEnt, Vector *pApex )
{
	...
	vecLeft = vecOrigin + ( vecForward * ( flDist + sizeX ) ) - vecDir * ( sizeX * 3 );
	vecRight = vecOrigin + ( vecForward * ( flDist + sizeX ) ) + vecDir * ( sizeX * 3 );
	if ( pev->movetype == MOVETYPE_FLY )
	{
		vecTop = vecOrigin + (vecForward * flDist) + (vecDirUp * sizeZ * 3);
		vecBottom = vecOrigin + (vecForward * flDist) - (vecDirUp *  sizeZ * 3);
	}
...
//	if ( CVAR_GET_FLOAT("stopmove" ) != 0 )
	...
	if ( (m_Route[ m_iRouteIndex ].iType & (~bits_MF_NOT_TO_MASK)) == bits_MF_TO_ENEMY )
		pTargetEnt = m_hEnemy; // only on a PURE move to enemy ( i.e., ONLY MF_TO_ENEMY set, not MF_TO_ENEMY and DETOUR )
...
//		REMOVE_ENTITY(ENT(pev));
		UTIL_Remove( this ); // diffusion - use this to properly remove monster's sprites/attachments etc.
		return;
	}
	...
	if( HasSpawnFlags( SF_MONSTER_DONTFORGET ) )
	...
	m_flDistTooClose	= 72.0;

	if( !HealthbarDisabled )
		EnableHealthBar = true;

	m_afCapability |= bits_CAP_CANSEEFLASHLIGHT; // if monsters must not have it, they should remove this after MonsterInit()

	// diffusion - friends can't be lit on fire with flares
...
void CBaseMonster :: StartMonster ( void )
{
	...
	if ( pev->movetype != MOVETYPE_FLY && !HasSpawnFlags(SF_MONSTER_FALL_TO_GROUND ) )
	{
		Vector vecOrigin = GetAbsOrigin();
		vecOrigin.z += 1;
		...
		if (!WALK_MOVE ( ENT(pev), 0, 0, WALKMOVE_NORMAL ) && !HasSpawnFlags(SF_MONSTER_NO_YELLOW_BLOBS))
		{
...
//		WorldGraph.m_iLastCoverSearch = nodeNumber + 1; // next monster that searches for cover node will start where we left off here. //freeslave
...
// Diffusion - NPC turning rate fix if higher than 60 fps!
// Thanks to SoloKiller.
//=========================================================

float CBaseMonster::ChangeYaw ( int yawSpeed )
{
	static float lasttime;
	float frametime = gpGlobals->time - lasttime;
...
const float originalSpeed = ( float ) yawSpeed * frametime * 10;
...
{ 
	...
	case CHAR_TEX_CONCRETE:
		...
		rgsz[0] = "footsteps/step1.wav";
		rgsz[1] = "footsteps/step3.wav";
		rgsz[2] = "footsteps/step2.wav";
		rgsz[3] = "footsteps/step4.wav";
		cnt = 4;
		break;
	case CHAR_TEX_METAL:
		fvol = fWalking ? 0.2 : 0.5;
		rgsz[0] = "footsteps/metal1.wav";
		rgsz[1] = "footsteps/metal3.wav";
		rgsz[2] = "footsteps/metal2.wav";
		rgsz[3] = "footsteps/metal4.wav";
		cnt = 4;
		break;
	case CHAR_TEX_DIRT:
		fvol = fWalking ? 0.25 : 0.55;
		rgsz[0] = "footsteps/dirt1.wav";
		rgsz[1] = "footsteps/dirt3.wav";
	...
	case CHAR_TEX_VENT:
		...
		rgsz[0] = "footsteps/duct1.wav";
		rgsz[1] = "footsteps/duct3.wav";
		rgsz[2] = "footsteps/duct2.wav";
		rgsz[3] = "footsteps/duct4.wav";
		cnt = 4;
		break;
	case CHAR_TEX_GRATE:
		fvol = fWalking ? 0.2 : 0.5;
		rgsz[0] = "footsteps/grate1.wav";
		rgsz[1] = "footsteps/grate3.wav";
		rgsz[2] = "footsteps/grate2.wav";
		rgsz[3] = "footsteps/grate4.wav";
		cnt = 4;
		break;
	case CHAR_TEX_TILE:
		fvol = fWalking ? 0.2 : 0.5;
		rgsz[0] = "footsteps/tile1.wav";
		rgsz[1] = "footsteps/tile3.wav";
		rgsz[2] = "footsteps/tile2.wav";
		rgsz[3] = "footsteps/tile4.wav";
		rgsz[4] = "footsteps/tile5.wav";
		cnt = 5;
		break;
	case CHAR_TEX_SLOSH:
		fvol = fWalking ? 0.2 : 0.5;
		rgsz[0] = "footsteps/slosh1.wav";
		rgsz[1] = "footsteps/slosh3.wav";
		rgsz[2] = "footsteps/slosh2.wav";
		rgsz[3] = "footsteps/slosh4.wav";
		cnt = 4;
		break;
	case CHAR_TEX_WOODSTEP:
		fvol = fWalking ? 0.2 : 0.5;
		rgsz[0] = "footsteps/wood1.wav";
		rgsz[1] = "footsteps/wood3.wav";
		rgsz[2] = "footsteps/wood2.wav";
		rgsz[3] = "footsteps/wood4.wav";
		cnt = 4;
		break;
	case CHAR_TEX_SNOW:
		fvol = fWalking ? 0.2 : 0.5;
		rgsz[0] = "footsteps/snow1.wav";
		rgsz[1] = "footsteps/snow3.wav";
		rgsz[2] = "footsteps/snow2.wav";
		rgsz[3] = "footsteps/snow4.wav";
		cnt = 4;
		break;
	case CHAR_TEX_CARPET:
		fvol = fWalking ? 0.2 : 0.5;
		rgsz[0] = "footsteps/carpet1.wav";
		rgsz[1] = "footsteps/carpet3.wav";
		rgsz[2] = "footsteps/carpet2.wav";
		rgsz[3] = "footsteps/carpet4.wav";
		cnt = 4;
		break;
	case CHAR_TEX_GRASS:
		fvol = fWalking ? 0.2 : 0.5;
		rgsz[0] = "footsteps/grass1.wav";
		rgsz[1] = "footsteps/grass3.wav";
		rgsz[2] = "footsteps/grass2.wav";
		rgsz[3] = "footsteps/grass4.wav";
		cnt = 4;
		break;
	case CHAR_TEX_GRAVEL:
		fvol = fWalking ? 0.2 : 0.5;
		rgsz[0] = "footsteps/gravel1.wav";
		rgsz[1] = "footsteps/gravel3.wav";
	...
	case CHAR_TEX_GLASS:
		fvol = fWalking ? 0.2 : 0.5;
		rgsz[0] = "footsteps/glass1.wav";
		rgsz[1] = "footsteps/glass3.wav";
		rgsz[2] = "footsteps/glass2.wav";
		rgsz[3] = "footsteps/glass4.wav";
		cnt = 4;
		break;
	default:	// default NPC sounds
		fvol = fWalking ? 0.2 : 0.5;
		rgsz[0] = "footsteps/step1.wav";
		rgsz[1] = "footsteps/step2.wav";
		rgsz[2] = "footsteps/step3.wav";
		rgsz[3] = "footsteps/step4.wav";
		cnt = 4;
		break;
	}
	...
	{
		fvol = fWalking ? 0.2 : 0.5;
		rgsz[0] = "footsteps/wade1.wav";
		rgsz[1] = "footsteps/wade3.wav";
		rgsz[2] = "footsteps/wade2.wav";
		rgsz[3] = "footsteps/wade4.wav";
		cnt = 4;
...
void CBaseMonster :: HandleAnimEvent( MonsterEvent_t *pEvent )
{
	...
	case SCRIPT_EVENT_SOUND_VOICE:
		if( !HasSpawnFlags(SF_MONSTER_GAG)) // diffusion - added check
...
void CBaseMonster::ReportAIState( void )
{
	...
	if ( (int)m_MonsterState < SIZEOFARRAY(pStateNames) )
...
void CBaseMonster :: KeyValue( KeyValueData *pkvd )
{
	...
	else if( FStrEq( pkvd->szKeyName, "m_flDistTooFar" ) ) // diffusion
	{
		m_flDistTooFar = Q_atof( pkvd->szValue );
		pkvd->fHandled = TRUE;
	}
	else if( FStrEq( pkvd->szKeyName, "m_flDistLook" ) ) // diffusion
	{
		m_flDistLook = Q_atof( pkvd->szValue );
		pkvd->fHandled = TRUE;
	}
	else if( FStrEq( pkvd->szKeyName, "nohealthbar" ) ) // diffusion
	{
...
*/
	...
	// diffusion - there's now 3 conditions. If one of them is met, the target will be activated.

	switch ( m_iTriggerCondition2 )
	{
	case AITRIGGER_SEEPLAYER_ANGRY_AT_PLAYER:
		if ( m_hEnemy != NULL && m_hEnemy->IsPlayer() && HasConditions ( bits_COND_SEE_ENEMY ) )
		{
...
void CBaseMonster::PlaySentence( const char *pszSentence, float duration, float volume, float attenuation, string_t SpeakFrom )
{
...
//=========================================================
// GetEnemy - tries to find the best suitable enemy for the monster.
//=========================================================
BOOL CBaseMonster :: GetEnemy ( void )
{
	...
	if ( m_hEnemy != NULL ) // monster has an enemy.
		return TRUE;
...
BOOL CBaseMonster :: ShouldFadeOnDeath( void )
{
	...
	if ( (pev->spawnflags & SF_MONSTER_FADECORPSE) || !FNullEnt( pev->owner ) || (sv_fadecorpses.value == 1) )
		return TRUE;
