...
#define	SF_MONSTER_GAG					BIT(1) // monster doesn't speak
#define SF_MONSTER_HITMONSTERCLIP		BIT(2)
//										BIT(3)
#define SF_MONSTER_PRISONER				BIT(4) // monster won't attack anyone, no one will attack him.
//										BIT(5)
//										BIT(6)
#define SF_MONSTER_NO_YELLOW_BLOBS		128 //LRC- if the monster is stuck, don't give errors or show yellow blobs.
#define SF_MONSTER_WAIT_FOR_SCRIPT		128 //spawnflag that makes monsters wait to check for attacking until the script is done or they've been attacked // diffusion - how is this used?
...
#define SF_MONSTER_NO_WPN_DROP			BIT(10) //LRC- never drop your weapon (player can't pick it up.)
#define	SF_MONSTER_NODAMAGE				BIT(11)	// diffusion - ignore all damage
...
#define	SF_MONSTER_WEAPON_LOCKED		BIT(16) // diffusion - can't pickup dropped weapon	
...
extern void EjectBrass (const Vector &vecOrigin, const Vector &vecVelocity, float rotation, int body, int soundtype );
...
#define IMPLEMENT_CUSTOM_SCHEDULES(derivedClass, baseClass)\
		...
		{\
			Schedule_t *pSchedule = ScheduleInList( pName, m_scheduleList, SIZEOFARRAY(m_scheduleList) );\
			if ( !pSchedule )\
