...
#define bits_NODE_LAND      BIT(0)  // Land node, so nudge if necessary.
#define bits_NODE_AIR       BIT(1)  // Air node, don't nudge.
#define bits_NODE_WATER     BIT(2)  // Water node, don't nudge.
#define bits_NODE_AIRBIG	BIT(3)  // diffusion - separate air nodes for bigger fly monsters (monster_alien_ship)
#define bits_NODE_LANDBIG	BIT(4)	// diffusion - land node, but for big monsters (robo)
#define bits_NODE_GROUP_REALM (bits_NODE_LAND | bits_NODE_LANDBIG | bits_NODE_AIR | bits_NODE_AIRBIG | bits_NODE_WATER )
