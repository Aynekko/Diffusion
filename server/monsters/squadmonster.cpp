...
void CSquadMonster :: SquadPasteEnemyInfo ( void )
{
	...
	if (pSquadLeader)
	{
		...
		pSquadLeader->m_flLastTimeObservedEnemy = gpGlobals->time; // freeslave
	}
}
...
void CSquadMonster :: StartMonster( void )
{
	...
	if ( ( m_afCapability & bits_CAP_SQUAD ) && !InSquad() )
	{
		if ( !FStringNull( pev->netname ) )
		{
			...
			if ( !( HasSpawnFlags(SF_SQUADMONSTER_LEADER) ))
				return;
		}

		// try to form squads now.
		// diffusion - increased radius (1024) and size
		int iSquadSize = SquadRecruit( 4096, MAX_SQUAD_MEMBERS - 1 );

		if ( iSquadSize )
		  ALERT ( at_aiconsole, "Squad \"%s\" of %d %s formed\n", STRING(pev->netname), iSquadSize, STRING( pev->classname ) );

		// diffusion - I'll remove this :)
