...
Schedule_t	slIdleStopShooting[] =
{
	{ 
		tlIdleStopShooting,
		SIZEOFARRAY ( tlIdleStopShooting ), 
		bits_COND_NEW_ENEMY		|
		bits_COND_LIGHT_DAMAGE	|
		bits_COND_HEAVY_DAMAGE	|
		bits_COND_HEAR_SOUND,
...
Schedule_t	slTlkIdleWatchClient[] =
{
	{ 
		...
		SIZEOFARRAY ( tlTlkIdleWatchClient ), 
		bits_COND_NEW_ENEMY		|
		bits_COND_LIGHT_DAMAGE	|
		bits_COND_HEAVY_DAMAGE	|
		bits_COND_HEAR_SOUND	|
	...
	{ 
		...
		SIZEOFARRAY ( tlTlkIdleWatchClientStare ), 
		bits_COND_NEW_ENEMY		|
		bits_COND_LIGHT_DAMAGE	|
		bits_COND_HEAVY_DAMAGE	|
		bits_COND_HEAR_SOUND	|
...
Schedule_t	slTlkIdleEyecontact[] =
{
	{ 
		tlTlkIdleEyecontact,
		SIZEOFARRAY ( tlTlkIdleEyecontact ), 
		bits_COND_NEW_ENEMY		|
		bits_COND_CLIENT_PUSH	|
		bits_COND_LIGHT_DAMAGE	|
		bits_COND_HEAVY_DAMAGE,
...
void CTalkMonster :: StartTask( Task_t *pTask )
{
	...
	case TASK_TLK_IDEALYAW:
		if (m_hTalkTarget != NULL)
		{
			...
			if (yaw < 0)
			{
				pev->ideal_yaw = Q_min( yaw + 45, 0 ) + GetAbsAngles().y;
			}
			else
			{
				pev->ideal_yaw = Q_max( yaw - 45, 0 ) + GetAbsAngles().y;
			}
		}
		TaskComplete();
		break;
...
int CTalkMonster :: FIdleHello( void )
{
	...
	if (!FBitSet(m_bitsSaid, bit_saidHelloPlayer))
	{
		...
		if (pPlayer)
		{
			if (FInViewCone(pPlayer, m_flFieldOfView) && FVisible(pPlayer))
			{
