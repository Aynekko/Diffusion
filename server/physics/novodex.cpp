...
void CPhysicNovodex :: InitPhysic( void )
{
	...
	worldBounds.min = NxVec3( -65536, -65536, -65536 );
	worldBounds.max = NxVec3(  65536,  65536,  65536 );
...
const NxBodyDesc*	body;			not needed
NxReal		density;			constant (???)
NxU32		flags;			m_iActorFlags;
NxActorGroup	group;			pev->groupinfo (???).
NxU32		contactReportFlags; 	not needed (already set by scene rules)
NxU16		forceFieldMaterial;		not needed (not used)
void*		userData;			not needed (automatically restored)
const char*	name;			not needed (automatically restored)
NxCompartment	*compartment;		not needed (not used)
NxActorDescType	type;			not needed (automatically restored) 
// body description
NxMat34		massLocalPose;		not needed (probably automatically recomputed on apply shape)
NxVec3		massSpaceInertia;		not needed (probably automatically recomputed on apply shape)
NxReal		mass;			not needed (probably automatically recomputed on apply shape)
NxVec3		linearVelocity;		pev->velocity
NxVec3		angularVelocity;		pev->avelocity
NxReal		wakeUpCounter;		not needed (use default)
NxReal		linearDamping;		not needed (use default)
NxReal		angularDamping;		not needed (use default)
NxReal		maxAngularVelocity; 	not needed (already set by scene rules)
NxReal		CCDMotionThreshold;		not needed (use default)
NxU32		flags;			m_iBodyFlags;
NxReal		sleepLinearVelocity; 	not needed (already set by scene rules)
NxReal		sleepAngularVelocity; 	not needed (already set by scene rules)
NxU32		solverIterationCount;	not needed (use default)
NxReal		sleepEnergyThreshold;	not needed (use default)
NxReal		sleepDamping;		not needed (use default)
NxReal		contactReportThreshold;	not needed (use default)
*/
...
int CPhysicNovodex :: BuildCollisionTree( char *szMapName )
{
	...
	for( i = 0; i < m_pWorldModel->nummodelsurfaces; i++ )
	{
		...
		if( psurf->flags & ( SURF_WATER|SURF_DRAWSKY ))
			continue;
