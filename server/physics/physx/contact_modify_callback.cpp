...
for each contact pair involving a conveyor entity, sets the contact target velocity so objects resting on the conveyor are carried along
=================
*/
void ContactModifyCallback::onContactModify(PxContactModifyPair* const pairs, PxU32 count)
{
	for (PxU32 i = 0; i < count; i++)
	{
		...
		if (!e1 || !e2)
		{
			continue; // was "return", which would silently drop the remaining pairs of the batch
		}
