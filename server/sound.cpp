...
*/
	...
	// start thinking yet.
//	SetThink( &CAmbientGeneric::ReInitialize );
//	DontThink();
	...
	if ( HasSpawnFlags(AMBIENT_SOUND_NOT_LOOPING) )
		m_fLooping = FALSE;
	else
		m_fLooping = TRUE;
...
void CAmbientGeneric :: Precache( void )
{
	...
	if ( !HasSpawnFlags(AMBIENT_SOUND_START_SILENT) )
	{
	...
	if( m_fActive )
	{
		...
		{
			if( g_pGameRules->IsMultiplayer() ) // diffusion - I had to do this. lmao!
				UTIL_EmitAmbientSound( edict(), GetAbsOrigin(), szSoundFile, (m_dpv.vol * 0.01), m_flAttenuation, SND_SPAWNING, m_dpv.pitch );
			else
				SetThink( &CAmbientGeneric::ReInitialize );
		}

		SetNextThink( RANDOM_FLOAT( 0.1, 1.0 ) ); // diffusion - don't start all sounds at the same time! sometimes it can crash on saverestore, if too many sounds.
	}
}
...
void CAmbientGeneric::ClearEffects(void)
{
...
void CAmbientGeneric :: KeyValue( KeyValueData *pkvd )
{
	...
	else if (FStrEq(pkvd->szKeyName, "attenuation"))
	{
		...
		if (m_flAttenuation > 3) m_flAttenuation = 3;
		if (m_flAttenuation < 0) m_flAttenuation = 0;
		pkvd->fHandled = TRUE;
	}
	else
...
//========================================================================
// FEnvSoundInRange: returns TRUE if the given sound entity (pev) is in range 
...
void SENTENCEG_Init()
{
	...
	while ( memfgets(pMemFile, fileSize, filePos, buffer, 511) != NULL )
	{
		...
		if (gcallsentences >= CVOXFILESENTENCEMAX) // diffusion - https://github.com/ValveSoftware/halflife/issues/3277
		{
...
void TEXTURETYPE_Init()
{
	...
	pMemFile = g_engfuncs.pfnLoadFileForMe( "data/materials.txt", &fileSize );
	if ( !pMemFile )
		return;

	// for each line in the file...
	while (memfgets(pMemFile, fileSize, filePos, buffer, sizeof(buffer) - 1) != NULL && (gcTextures < CTEXTURESMAX)) // 511
	{
		// skip whitespace
		i = 0;
		/*
	...
	//		continue;
		...
		// null-terminate name and save in textures array
...
//		return 0.0;
	...
	{
			// diffusion monster specific (ugly hacks? I'm sorry)
			if( pEntity->HasSpawnFlags(SF_MONSTER_NODAMAGE) )
				chTextureType = CHAR_TEX_NONE;
			else
			{
				// diffusion - bad!!!!!
				if (pEntity->HasFlag(F_METAL_MONSTER) )
					chTextureType = CHAR_TEX_NONE; // I use different metal sounds in monster...
				else if (FClassnameIs(pEntity->pev, "monster_alice"))
					chTextureType = CHAR_TEX_NONE;
				else
					// hit body
	...
	case CHAR_TEX_CONCRETE: fvol = 0.9;	fvolbar = 0.6;
		rgsz[0] = "hits/concrete1.wav";
		rgsz[1] = "hits/concrete2.wav";
		rgsz[2] = "hits/concrete3.wav";
		rgsz[3] = "hits/concrete4.wav";
		cnt = 4;
		break;
	case CHAR_TEX_GRATE:
	case CHAR_TEX_VENT:
	case CHAR_TEX_METAL: fvol = 0.9; fvolbar = 0.3;
		rgsz[0] = "hits/metal1.wav";
		rgsz[1] = "hits/metal2.wav";
		rgsz[2] = "hits/metal3.wav";
		rgsz[3] = "hits/metal4.wav";
		cnt = 4;
		break;
	case CHAR_TEX_GRASS:
	...
	case CHAR_TEX_DIRT:	fvol = 0.9; fvolbar = 0.1;
		rgsz[0] = "hits/dirt1.wav";
		rgsz[1] = "hits/dirt2.wav";
		rgsz[2] = "hits/dirt3.wav";
		rgsz[3] = "hits/dirt3.wav";
		cnt = 4;
		DoRicochet = false;
		break;
//	case CHAR_TEX_VENT:	fvol = 0.5; fvolbar = 0.3; // use metal
//		rgsz[0] = "player/pl_duct1.wav";
//		rgsz[1] = "player/pl_duct1.wav";
//		cnt = 2;
//		break;
//	case CHAR_TEX_GRATE: fvol = 0.9; fvolbar = 0.5; // use metal
//		rgsz[0] = "player/pl_grate1.wav";
//		rgsz[1] = "player/pl_grate4.wav";
//		cnt = 2;
//		break;
//	case CHAR_TEX_TILE:	fvol = 0.8; fvolbar = 0.2; // use default for now
...
//		break;
	...
	case CHAR_TEX_WOOD: fvol = 0.9; fvolbar = 0.2;
		rgsz[0] = "hits/wood1.wav";
		rgsz[1] = "hits/wood2.wav";
		rgsz[2] = "hits/wood3.wav";
		rgsz[3] = "hits/wood4.wav";
		cnt = 4;
		DoRicochet = false;
		break;
	case CHAR_TEX_GLASS:
	case CHAR_TEX_COMPUTER:
		...
		rgsz[0] = "hits/glass1.wav";
		rgsz[1] = "hits/glass2.wav";
		rgsz[2] = "hits/glass3.wav";
		rgsz[3] = "hits/glass4.wav";
		cnt = 4;
		break;
	case CHAR_TEX_FLESH:
		...
		{
			// sounds precached in crowbar
			rgsz[0] = "weapons/cbar_hitbod1.wav";
			rgsz[1] = "weapons/cbar_hitbod2.wav";
			rgsz[2] = "weapons/cbar_hitbod3.wav";
			cnt = 3;
		}
		else
		{
		...
		DoRicochet = false;
		break;
	case CHAR_TEX_NONE: // usually it's a sky texture
		if (iBulletType == BULLET_PLAYER_CROWBAR)
		{
			fvol = 1.0;	fvolbar = 0.2;
			// sound precached in crowbar
			rgsz[0] = "weapons/cbar_miss1.wav";
			cnt = 1;
			fattn = 1.0;
		}
		else
	...
	if ( g_pGameRules->IsMultiplayer() ) // UNDONE: maybe should be a cvar?
		fvol *= 0.5;

	// play material hit sound
	UTIL_EmitAmbientSound(ENT(0), ptr->vecEndPos, rgsz[RANDOM_LONG(0,cnt-1)], fvol, fattn, 0, 96 + RANDOM_LONG(0,6));

	// diffusion - no ricochet effects under water
	if( UTIL_PointContents( ptr->vecEndPos ) == CONTENTS_WATER )
		DoRicochet = false;

	// diffusion - ricochet sounds moved from the engine to here
	if( (iBulletType != BULLET_PLAYER_CROWBAR) && DoRicochet )
	{
		if (RANDOM_LONG(0,3) == 0)
		{
			...
			{
				case 0: UTIL_EmitAmbientSound(ENT(0), ptr->vecEndPos, "hits/ric1.wav", fvol, fattn, 0, 96 + RANDOM_LONG(0,0xf)); break;
				case 1: UTIL_EmitAmbientSound(ENT(0), ptr->vecEndPos, "hits/ric2.wav", fvol, fattn, 0, 96 + RANDOM_LONG(0,0xf)); break;
				case 2: UTIL_EmitAmbientSound(ENT(0), ptr->vecEndPos, "hits/ric3.wav", fvol, fattn, 0, 96 + RANDOM_LONG(0,0xf)); break;
				case 3: UTIL_EmitAmbientSound(ENT(0), ptr->vecEndPos, "hits/ric4.wav", fvol, fattn, 0, 96 + RANDOM_LONG(0,0xf)); break;
				case 4: UTIL_EmitAmbientSound(ENT(0), ptr->vecEndPos, "hits/ric5.wav", fvol, fattn, 0, 96 + RANDOM_LONG(0,0xf)); break;
			}

			TraceResult tr = UTIL_GetGlobalTrace( );
			Vector vecOrg = ptr->vecEndPos + tr.vecPlaneNormal * 3;

			if( gpGlobals->maxClients == 1 )  // light from ricochets (singleplayer only)
			{
