...
// diffusion - code from BugFixedHL by Lev
...
void UTIL_ScreenShake( const Vector &center, float amplitude, float frequency, float duration, float radius, bool bAllowInAir )
{
	...
	for ( i = 1; i <= gpGlobals->maxClients; i++ )
	{
		...
		if ( localAmplitude )
		{
			...
			MESSAGE_BEGIN( MSG_ONE, gmsgTempEnt, NULL, pPlayer->edict() );		// use the magic #1 for "one client"
				
				WRITE_BYTE( TE_SCREENSHAKE );
...
void UTIL_ScreenShakeLocal(CBaseEntity *pEntity, const Vector& center, float amplitude, float frequency, float duration, float radius, bool bAllowInAir )
{
	...
	if (!pEntity || (!FBitSet(pEntity->pev->flags, FL_ONGROUND) && !bAllowInAir)) // Don't shake if not onground
		return;
	...
	{
		Vector delta = center - pEntity->GetAbsOrigin();
		float distance = delta.Length();
	...
	if (localAmplitude)
	{
		...
		MESSAGE_BEGIN(MSG_ONE, gmsgTempEnt, NULL, pEntity->edict());		// use the magic #1 for "one client"
			WRITE_BYTE( TE_SCREENSHAKE );
			WRITE_SHORT(shake.amplitude);		// shake amount
			WRITE_SHORT(shake.duration);		// shake lasts this long
			WRITE_SHORT(shake.frequency);		// shake noise frequency
...
void UTIL_HudMessage( CBaseEntity *pEntity, const hudtextparms_t &textparms, const char *pMessage )
{
	...
	const int MAX_LENGTH = 235; // diffusion - was 512, but it's crashing the server. this value works well enough

	// diffusion - safety check
	if( strlen( pMessage ) > MAX_LENGTH )
		ALERT( at_warning, "UTIL_HudMessage: Message is too long!\n" );
	...
	MESSAGE_BEGIN( MSG_ONE, SVC_TEMPENTITY, NULL, pEntity->edict() );
		...
		{
			char tmp[MAX_LENGTH];
			strncpy( tmp, pMessage, MAX_LENGTH - 1 );
			tmp[MAX_LENGTH - 1] = 0;
			WRITE_STRING( tmp );
		}
	MESSAGE_END();
...
void UTIL_ShowMessage( const char *pString, CBaseEntity *pEntity )
{
	...
	MESSAGE_BEGIN( MSG_ONE, gmsgHudText, NULL, pEntity->edict() ); //gmsgHudText
...
{
	if( speed == 0.0f || value == target ) // diffusion
		return value;
	...
	do
	{
		if(!( pTarget->pev->flags & FL_KILLME ))
		{
			...
			else
			{
				...
				// diffusion - moved alert here
				ALERT( at_aiconsole, "Firing: %s (%s) with %s and value %g\n", pTarget->GetClassname(), targetName, GetStringForUseType( useType ), value );
			}
		}
...
void UTIL_BloodDrips( const Vector &origin, const Vector &direction, int color, int amount )
{
	...
	*/
	...
	// diffusion - new particles
	Vector ColorVec = Vector( 127, 0, 0 );// Vector( 136, 8, 8 ); // red by default
	if( color == BLOOD_COLOR_YELLOW )
		ColorVec = Vector( 220, 220, 0 );

	MESSAGE_BEGIN( MSG_PVS, gmsgTempEnt, origin );
	WRITE_BYTE( TE_BLOODSPRITE );
	WRITE_COORD( origin.x );								// pos
	WRITE_COORD( origin.y );
	WRITE_COORD( origin.z );
	WRITE_BYTE( ColorVec.x );	
	WRITE_BYTE( ColorVec.y );
	WRITE_BYTE( ColorVec.z );
	WRITE_COORD( direction.x );
	WRITE_COORD( direction.y );
	WRITE_COORD( direction.z );
	WRITE_BYTE( Q_min( Q_max( 3, amount / 10 ), 16 ) );		// size
...
{
	return; // diffusion - studiodecalsfpslossissue
	/*
	short entityIndex;
	int decalIndex;
...
void UTIL_GunshotDecalTrace( TraceResult *pTrace, int decalNumber )
{
	...
	MESSAGE_BEGIN( MSG_PAS, gmsgTempEnt, pTrace->vecEndPos );
...
{
	MESSAGE_BEGIN( MSG_PVS, gmsgTempEnt, position );
...
void UTIL_DoSpark( entvars_t *pev, const Vector &location )
{
	...
	float flVolume = RANDOM_FLOAT( 0.25f, 0.75f ) * 0.75f; // random volume range
...
//diffusion hacks...
void UTIL_DoSparkNoSound( entvars_t *pev, const Vector &location )
{
	Vector tmp = location + pev->size * 0.5;
...
void UTIL_Ricochet( const Vector &position, float scale )
{
	...
	Vector newposition = position + GodDamnTrace.vecPlaneNormal * 3; // pull out from the touched surface
	
	MESSAGE_BEGIN( MSG_PVS, SVC_TEMPENTITY, position );
		WRITE_BYTE( TE_ARMOR_RICOCHET );
		WRITE_COORD( newposition.x );
		WRITE_COORD( newposition.y );
		WRITE_COORD( newposition.z );
...
unsigned short UTIL_PrecacheMovie( const char *s, int allow_sound )
{
	...
	{
		ALERT( at_console, "^3Error:^7 video (%s) name too long\n", path );
		return 0;
	}
...
hull_t *UTIL_HullForBsp( CBaseEntity *pEntity, const Vector &mins, const Vector &maxs, Vector &offset )
{
	...
	if( !model || model->type != mod_brush )
		HOST_ERROR( "Entity %i SOLID_BSP with a non bsp model %i (name: %s)\n", pEntity->entindex(), model->type, pEntity->GetTargetname() );
	...
	if( !waterlevel )
	{
		if( flags & FL_INWATER )
		{
			...
			case 1:
				...
				break;
			case 2:
				EMIT_SOUND( pEntity->edict(), CHAN_BODY, "footsteps/wade3.wav", 1.0f, ATTN_NORM );
				break;
			case 3:
				EMIT_SOUND( pEntity->edict(), CHAN_BODY, "footsteps/wade4.wav", 1.0f, ATTN_NORM );
				break;
			}
