...
typedef enum
{
	...
	STATE_ON,		// enabled, active, visible, or open. Or alert monster.
...
// diffusion
inline BOOL FIStrEq( const char *sz1, const char *sz2 )
{
...
extern void		UTIL_DoSparkNoSound( entvars_t *pev, const Vector &location ); //diffusion
...
// SND flags are duplicated in protocol.h and on client in utils.h
...
#define PRECACHE_SOUND_ARRAY( a ) \
	{ for (int i = 0; i < SIZEOFARRAY( a ); i++ ) PRECACHE_SOUND((char *) a [i]); }

#define EMIT_SOUND_ARRAY_DYN( chan, array ) \
	EMIT_SOUND_DYN ( ENT(pev), chan , array [ RANDOM_LONG(0,SIZEOFARRAY( array )-1) ], 1.0, ATTN_NORM, 0, RANDOM_LONG(95,105) ); 

#define RANDOM_SOUND_ARRAY( array ) (array) [ RANDOM_LONG(0,SIZEOFARRAY( (array) )-1) ]
...
// diffusion - added here
inline bool UTIL_FileExists( char *filename )
{
	int iCompare;
...
inline bool UTIL_SoundExists( char *filename )
{
	if( strlen( filename ) > 80 )
		return false;
	
	char AddPath[7] = "sound/";
	char NewPath[100] = "";
	sprintf( NewPath, "%s%s", AddPath, filename );

	// verify file exists
	if( UTIL_FileExists( NewPath ) )
		return true;
	return false;
}
