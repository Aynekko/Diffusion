...
// diffusion - UNDONE since I got rid of skill.cfg for weapons, the damage is now at fixed value of 5
...
//LINK_ENTITY_TO_CLASS( weapon_egon, CEgon );  not used
...
void CEgon::Attack( void )
{
	...
	switch( m_fireState )
	{
		case FIRE_OFF:
		{
			...
			SendWeaponAnim( g_fireAnims1[ RANDOM_LONG(0,SIZEOFARRAY(g_fireAnims1)-1) ] );
			m_shakeTime = 0;
...
void CEgon::Fire( const Vector &vecOrigSrc, const Vector &vecDir )
{
	...
	case FIRE_NARROW:
		if ( pev->dmgtime < gpGlobals->time )
		{
			...
			{
				pEntity->TraceAttack( m_pPlayer->pev, 5, vecDir, &tr, DMG_ENERGYBEAM );
			}
	...
	case FIRE_WIDE:
		if ( pev->dmgtime < gpGlobals->time )
		{
			...
			if (pEntity->pev->takedamage)
			{
				pEntity->TraceAttack( m_pPlayer->pev, 5, vecDir, &tr, DMG_ENERGYBEAM | DMG_ALWAYSGIB);
			}
			ApplyMultiDamage(m_pPlayer->pev, m_pPlayer->pev);
			...
			if ( g_pGameRules->IsMultiplayer() )
			{
				...
				::RadiusDamage( tr.vecEndPos, pev, m_pPlayer->pev, 1, 128, CLASS_NONE, DMG_ENERGYBEAM | DMG_BLAST | DMG_ALWAYSGIB );
			}
