...
DLL_GLOBAL short g_sBlastTexture;
...
//==================================================================
void PlayPickupSound( CBaseEntity *pPlayer, int Id )
{
	if( !pPlayer )
		return;
	
	Vector org = pPlayer->GetAbsOrigin();
	MESSAGE_BEGIN( MSG_ONE, gmsgTempEnt, org, pPlayer->pev );
	WRITE_BYTE( TE_CLIENTSOUND );
	WRITE_COORD( org.x );
	WRITE_COORD( org.y );
	WRITE_COORD( org.z );
	WRITE_SHORT( (short)ENTINDEX( pPlayer->edict() ) );
	WRITE_BYTE( Id );
	WRITE_BYTE( 0 );
	WRITE_BYTE( 0 );
	MESSAGE_END();
...
void MakeWeaponShake( CBaseEntity *pPlayer, int Weapon, int Mode )
{
	...
	MESSAGE_BEGIN( MSG_ONE, gmsgTempEnt, org, pPlayer->pev );
	WRITE_BYTE( TE_WEAPONSHAKE );
	WRITE_BYTE( Weapon );
	WRITE_BYTE( Mode );
	MESSAGE_END();
}

void PlayClientSound( CBaseEntity *pEntity, int Type, int Mode, int LowAmmoVolume, Vector org )
{
	if( !pEntity )
		return;

	if( !org || org == g_vecZero )
		org = pEntity->GetAbsOrigin();
	...
	WRITE_COORD( org.y );
	WRITE_COORD( org.z );
	WRITE_SHORT( (short)ENTINDEX( pEntity->edict() ) );
	WRITE_BYTE( Type );
	WRITE_BYTE( Mode );
	WRITE_BYTE( LowAmmoVolume ); // x0.1
	MESSAGE_END();
}
...
void DecalGunshot( TraceResult *pTrace, int iBulletType )
{
	...
	if( UTIL_GetModelType( pev->modelindex ) == mod_brush )
	{
		...
		case BULLET_PLAYER_12MM:
			// smoke and decal
			UTIL_GunshotDecalTrace( pTrace, DamageDecal( pEntity, DMG_BULLET ) );
			break;
		case BULLET_PLAYER_CROWBAR:
			// wall decal
			UTIL_GunshotDecalTrace( pTrace, DamageDecal( pEntity, DMG_SLASH ) );
			break;
		}
	}
...
//=================================================================================
// AddAmmoNameToAmmoRegistry: precaches the ammo and queues the ammo info for sending to clients
//=================================================================================
...
//=================================================================================
// UTIL_PrecacheOtherWeapon: precaches the weapon and queues the weapon info for sending to clients
//=================================================================================
...
//=================================================================================
// W_Precache: general precache of items during world spawn.
//=================================================================================
...
//	UTIL_PrecacheOther( "item_longjump" );
	...
	UTIL_PrecacheOtherWeapon( "weapon_shotgun_xm" );
...
//	UTIL_PrecacheOtherWeapon( "weapon_hornetgun" );
	...
	// and a sentry!
	UTIL_PrecacheOtherWeapon( "weapon_sentry" );
...
//	DEFINE_FIELD( m_flTimeWeaponIdle, FIELD_TIME ), // diffusion - experiment, do not save this, allow idle anim after saverestore
...
{
	if( !HasSpawnFlags( SF_AMMO_DONTFALL ) ) // diffusion - stop falling through the floor!!!
...
void CBasePlayerItem::FallThink ( void )
{
	...
	if ( (pev->flags & FL_ONGROUND) || HasSpawnFlags( SF_AMMO_DONTFALL ) )
	{
		// clatter if we have an owner (i.e., dropped by someone)
		// don't clatter if the gun is waiting to respawn (if it's waiting, it is invisible!)
		// diffusion - don't clatter if the gun is set to be in fixed position
		...
		// diffusion - it's suspended in air, so we may rotate our gun how we want :)
		if( !HasSpawnFlags( SF_AMMO_DONTFALL ) )
		{
			// lie flat
			angles.x = 0;
			angles.z = 0;
	...
	// diffusion - rotate while falling
	angles.x += 50 * gpGlobals->frametime;
...
CBaseEntity* CBasePlayerItem::Respawn( void )
{
	...
	if ( pNewWeapon )
	{
		pNewWeapon->pev->spawnflags = pev->spawnflags;
		...
		{
			pNewWeapon->pev->movetype = 0;
			Vector RestoreOrg = GetAbsOrigin();
			pNewWeapon->SetAbsOrigin( RestoreOrg );
		}
...
void CBasePlayerItem::DefaultTouch( CBaseEntity *pOther )
{
	...
	//	EMIT_SOUND(ENT(pPlayer->pev), CHAN_ITEM, "items/gunpickup2.wav", 1, ATTN_NORM);
		// diffusion - play sound on client, and for client only
		PlayPickupSound( pPlayer, 251 );
	}
...
int CBasePlayerItem::AddToPlayer( CBasePlayer *pPlayer )
{
	...
	// diffusion - check if we need to show a tutorial sprite on how to use the weapon.
	pPlayer->CheckTutorMessage( m_iId );
	// diffusion - after that, make a permanent mark that we don't need to show the tutor again
	pPlayer->HadWeapon[m_iId] = true;
...
int CBasePlayerWeapon::UpdateClientData( CBasePlayer *pPlayer )
{
	...
	if ( m_iClip != m_iClientClip || 
		 ...
		 (int)pPlayer->m_flFOV != pPlayer->m_iClientFOV )
	{
		bSend = TRUE;
	}
...
//		skiplocal = 0;
	...
	MESSAGE_BEGIN( MSG_ONE, gmsgWeaponAnim, NULL, m_pPlayer->pev );
		...
		WRITE_BYTE( m_iId ); // diffusion - also send weapon info...
	MESSAGE_END();
}
...
BOOL CBasePlayerWeapon :: AddPrimaryAmmo( int iCount, char *szName, int iMaxClip, int iMaxCarry )
{
	...
	else if (m_iClip == 0 && !m_pPlayer->HasPlayerItem( this ) ) // diffusion - added weapon check: fixed a bug when player picks up a weapon with empty clip, which leads to instant reloading
	{
		int i;
...
{
	if( m_pPlayer->CanSelectEmptyWeapon )
		return TRUE;
	...
	if ( m_iClip <= 0 )
	{
		if ( m_pPlayer->m_rgAmmo[ PrimaryAmmoIndex() ] <= 0 && iMaxAmmo1() != -1 )			
		{
			...
			if ( m_pPlayer->m_rgAmmo[ SecondaryAmmoIndex() ] <= 0  ) // diffusion - also check secondary ammo
				return FALSE;
		}
	}
...
BOOL CBasePlayerWeapon :: DefaultReload( int iClipSize, int iAnim, float fDelay, int body )
{
	...
	m_pPlayer->SetAnimation( PLAYER_RELOAD ); // nextday
...
void CBasePlayerWeapon::LowAmmoMsg(CBasePlayer* pPlayer)
{
	if( gpGlobals->time > pPlayer->HUDtextTime + HUD_TEXT_DELAY )
	{
		...
		pPlayer->HUDtextTime = gpGlobals->time; // should be set only once
	}
}
...
void CLaserSpot::Spawn( void )
{
	...
	pev->renderamt = 255;
	pev->scale = 0.1; // Diffusion: fix for better look :)
